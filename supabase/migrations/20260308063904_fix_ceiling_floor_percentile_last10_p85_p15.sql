/*
  # Fix ceiling_estimate and floor_estimate to use last-10 P85/P15

  ## Summary
  Updates v_neeko_player_projection (the authoritative player projection view in
  the afl schema) to change ceiling_estimate and floor_estimate calculations:

  ## Changes
  - ceiling_estimate: PERCENTILE_CONT(0.85) over last 10 games (was P90 over last 15)
  - floor_estimate:   PERCENTILE_CONT(0.15) over last 10 games (was P10 over last 15)

  ## Why
  Using last-10 games gives a more current upside/downside picture.
  P85 ceiling = "their best realistic score in a good week" (not extreme outliers).
  P15 floor = "their likely worst score in a tough week" (not extreme outliers).
  Both remain fully independent from projection_final.

  ## What is NOT changed
  - projection_final formula — unchanged
  - rolling projection weights — unchanged
  - All other metrics (avg_last_3, avg_last_5, volatility, etc.) — unchanged

  ## Safe approach
  Uses CREATE OR REPLACE VIEW — no data loss, instant swap.
*/
...
fixture_rows AS (
  SELECT round_number, match_date, venue, home_team AS team, away_team AS opponent, true AS is_home
  FROM next_fixtures
  UNION ALL
  SELECT round_number, match_date, venue, away_team AS team, home_team AS opponent, false AS is_home
  FROM next_fixtures
),
baseline_2025 AS (
  SELECT player_id,
...
rolling AS (
  SELECT
    ...
    round(stddev_pop(CASE WHEN row_num <= 15 THEN normalized_score ELSE NULL END), 2) AS volatility_last_15,

    -- Est. Floor: P15 over last 10 raw fantasy scores
    round((
      percentile_cont(0.15) WITHIN GROUP (ORDER BY
        CASE WHEN row_num <= 10 THEN fantasy_points::double precision ELSE NULL END
      )
    )::numeric, 1) AS floor_estimate,

    -- Est. Ceiling: P85 over last 10 raw fantasy scores
    round((
    ...
    round(NULLIF(sum(
      CASE WHEN row_num <= 15 THEN normalized_score * CASE row_num
        WHEN 1 THEN 1.00 WHEN 2 THEN 0.90 WHEN 3 THEN 0.80 WHEN 4 THEN 0.70 WHEN 5 THEN 0.60
        WHEN 6 THEN 0.50 WHEN 7 THEN 0.40 WHEN 8 THEN 0.35 WHEN 9 THEN 0.30 WHEN 10 THEN 0.25
        ELSE 0.15 END ELSE NULL END
    ), 0) / NULLIF(sum(
      CASE WHEN row_num <= 15 AND normalized_score IS NOT NULL THEN CASE row_num
        WHEN 1 THEN 1.00 WHEN 2 THEN 0.90 WHEN 3 THEN 0.80 WHEN 4 THEN 0.70 WHEN 5 THEN 0.60
        WHEN 6 THEN 0.50 WHEN 7 THEN 0.40 WHEN 8 THEN 0.35 WHEN 9 THEN 0.30 WHEN 10 THEN 0.25
        ELSE 0.15 END ELSE NULL END
    ), 0), 2) AS weighted_recent_avg
...
player_stats AS (
  SELECT
    ...
    r.avg_last_3, r.avg_last_5, r.avg_last_10, r.avg_last_15,
    r.volatility_last_15, r.floor_estimate, r.ceiling_estimate,
    ...
    CASE WHEN COALESCE(g.games_played_2026, 0) > 0
...
projections AS (
  SELECT ps.*,
    round(
      ...
      + (0.10 * ps.season_avg_current), 2) AS rolling_projection,
    CASE
...
SELECT
  b.player_id, b.player_name, b.team,
  f.opponent, f.venue, f.is_home, f.match_date,
  COALESCE(f.round_number, (SELECT rn FROM next_round_num)) AS target_round_number,
  b.season_context,
  b.games_played_2025, b.baseline_avg_2025,
  b.games_played_2026, b.season_avg_current,
  b.avg_last_5, b.avg_last_15, b.volatility_last_15,
  b.floor_estimate, b.ceiling_estimate,
  b.prob_100_plus, b.prob_120_plus, b.trend_3_vs_10,
  b.rolling_projection, b.final_projection,
