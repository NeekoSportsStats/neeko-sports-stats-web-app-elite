/*
  # Trade Engine V2c — Balanced Diversity (Final)

  ## Summary
  Refined diversity strategy: each buy player appears at most once, but sell players
  may repeat up to 3 times. This produces a list of 20+ varied trades without
  being over-constrained.

  ## Approach
  - PARTITION BY buy_player_id → each buy target appears once (strict)
  - After dedup, add sell_rank within the result set and limit to 3 per sell player
  - Ensures both buy and sell variety while keeping the list populated
*/
...
-- Each sell player appears at most 3 times in final list
sell_capped AS (
  SELECT *,
