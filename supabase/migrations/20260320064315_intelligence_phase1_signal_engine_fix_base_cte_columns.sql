...
/*
  # Phase 1d: Signal Engine — Fix base CTE missing columns

  Adds recommendation_short to the base CTE and drops the duplicate
  s2..s24 alias issue (each sub-CTE was re-aliasing v_snapshot_id
  as the column name instead of using the outer variable correctly).
  
  Complete rewrite with correct CTE column structure.
*/
...
BEGIN
  ...
  sigs AS (
    -- VALUE: undervalued
    SELECT player_id, v_snap AS snap,
      'undervalued'::text AS sig_type,
      LEAST(100,GREATEST(0, 50+(value_score-v_vs_p75)*3))::numeric(5,2) AS score,
      CASE WHEN value_score>=v_vs_p75*1.5 THEN 'strong'
           WHEN value_score>=v_vs_p75*1.2 THEN 'moderate' ELSE 'weak' END::text AS strength,
      'positive'::text AS direction,
      'Projection of '||ROUND(proj::numeric,0)||' pts exceeds price of $'||(price/1000)||'k — strong value.'::text AS expl,
      LEAST(1.0,GREATEST(0,0.5+(value_score-v_vs_p75)/50.0))::numeric(4,3) AS conf,
      jsonb_build_object('value_score',value_score,'price',price,'proj',ROUND(proj::numeric,1)) AS meta
    FROM base WHERE COALESCE(value_score,0)>=v_vs_p75 AND COALESCE(price,0)>0

    UNION ALL
    -- VALUE: overvalued
    SELECT player_id, v_snap,'overvalued',
    ...
    SELECT player_id, v_snap,'price_momentum',
  ...
  SELECT player_id, snap, sig_type, score, strength, direction, expl, conf, meta
  FROM sigs
  WHERE player_id IS NOT NULL
  ON CONFLICT (player_id, sig_type,
  ...
  RETURN jsonb_build_object(
    'ok',true,'signals_written',v_written,'players_processed',v_players,
    'snapshot_id',v_snap,'signal_types',24,'generated_at',NOW()
  );
