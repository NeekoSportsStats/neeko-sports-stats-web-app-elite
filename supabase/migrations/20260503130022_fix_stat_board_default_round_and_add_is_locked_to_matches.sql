/*
  # Fix Stat Board RPCs — Default Round + is_locked on Matches

  ## Changes

  ### 1. get_stat_board_matches
  - Added `is_locked boolean` to RETURNS TABLE (= NOT is_free_match = match_order > 2)
  - Fixed resolved_round: default now uses MAX(week) FROM afl.player_games
    (returns week 8, actual data) instead of MAX(week) FROM afl.games (returns week 24, future fixtures)
  - Must DROP + recreate because the return type changes

  ### 2. get_stat_board_players
  - Fixed resolved_round: same fix — MAX(week) FROM afl.player_games WHERE season = p_season
  - No return type change so CREATE OR REPLACE works
*/

-- ─────────────────────────────────────────────────────────────────────────────
-- 1. get_stat_board_matches: drop + recreate with is_locked + fixed round
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- 2. get_stat_board_players: fix resolved_round only (no return type change)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ── 2. Resolve selected round — use player_games for actual data max ─────────
resolved_round AS (
  SELECT COALESCE(
    p_round,
    (SELECT MAX(week) FROM afl.player_games WHERE season = p_season)
