/*
  # Fix run_neeko_ai_pipeline duration tracking

  ## Summary
  Two issues fixed in run_neeko_ai_pipeline:

  1. pipeline_runs INSERT used now() for started_at — since now() is fixed at transaction
     start, and the UPDATE also uses now(), all runs show 0-second duration.
     Fixed: capture clock_timestamp() into v_run_start before INSERT, use it for started_at.

  2. The final UPDATE sets finished_at = now() but never sets duration_ms.
     Fixed: use clock_timestamp() for finished_at and calculate duration_ms.

  ## Changes
  - run_neeko_ai_pipeline: v_run_start := clock_timestamp() before INSERT
  - INSERT uses v_run_start for started_at (not now())
  - Final UPDATE uses clock_timestamp() for finished_at and sets duration_ms
*/
...
VALUES (
  ...
  3, 0, 'Starting', v_run_start, v_run_start
)
