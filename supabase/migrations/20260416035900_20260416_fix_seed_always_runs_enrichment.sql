/*
  # Fix: Seed function now always runs enrichment pass

  ## Problem
  afl.fn_populate_player_rankings_cache() (the seed) can be called standalone
  from the admin panel or other contexts outside run_neeko_pipeline(). When this
  happens, the enrichment pass (afl.populate_rankings_cache()) never runs, leaving
  the seed's SMASH_START/HARD_SIT vocabulary as the final state of action_canonical.

  The enrichment maps these to the canonical START/SIT vocabulary and also
  rewrites signal_tag, signal_canonical, signal_display, category_canonical, and
  market_watch_category using the edge-based signal system.

  ## Fix
  Add a call to afl.populate_rankings_cache() at the end of
  fn_populate_player_rankings_cache() so the enrichment always follows the seed,
  regardless of how the seed is invoked.

  ## Impact
  - action_canonical will always reflect the edge-based enrichment vocabulary
    (START/SIT/HOLD) after any seed call
  - SMASH_START/HARD_SIT will no longer persist in live data after standalone runs
  - The run_neeko_pipeline() step 8 enrichment is now redundant but harmless
    (runs 0-change update pass)
*/
...
-- Always run enrichment pass to apply canonical edge-based signal vocabulary.
-- This ensures action_canonical always reflects the edge-based system (START/SIT/HOLD)
-- regardless of whether this function is called from the pipeline or standalone.
