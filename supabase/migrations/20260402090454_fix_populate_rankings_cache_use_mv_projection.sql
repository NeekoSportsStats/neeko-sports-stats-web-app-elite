/*
  # Fix Populate Rankings Cache - Use Materialized View
  
  ## Root Cause
  - Function references `afl.player_projection` table which has minimal columns
  - Should use `afl.mv_player_projection` materialized view with full metrics
  - Column mismatches: form_score, value_score, captain_score
  
  ## Solution
  - Switch to mv_player_projection
  - Map columns correctly (projection_final → projection, etc)
  - Calculate captain_score inline (not in source table)
