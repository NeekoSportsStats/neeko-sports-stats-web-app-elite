/*
  # Fix get_player_detail_safe RPC — subscription join and profile column

  ## Problem
  Two bugs cause "column ss.user_id does not exist" and related errors:

  1. Premium check queried `public.stripe_subscriptions` with `ss.user_id` — but
     `stripe_subscriptions` has no `user_id` column (only `customer_id`).
     The correct source of truth is `public.subscriptions` which has both
     `profile_id` and `user_id` columns.

  2. Manual premium check used `pr.manual_premium` — but the actual column in
     `public.profiles` is `is_manual_premium`.

  ## Fix
  - Replace `stripe_subscriptions ss WHERE ss.user_id = p_user_id` with
    `subscriptions s WHERE (s.profile_id = p_user_id OR s.user_id = p_user_id)`
    and check `s.current_period_end > now()` (matches is_premium_user() canonical).
  - Replace `pr.manual_premium` with `pr.is_manual_premium`.

  ## Safety
  - RPC still works for anon (p_user_id IS NULL → v_is_premium stays false)
  - RPC still works for premium users (correct table join)
...
GRANT EXECUTE ON FUNCTION public.get_player_detail_safe(text, uuid) TO anon, authenticated;
