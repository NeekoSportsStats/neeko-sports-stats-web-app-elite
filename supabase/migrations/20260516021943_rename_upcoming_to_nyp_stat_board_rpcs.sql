/*
  # Rename 'upcoming' to 'nyp' in Stat Board RPCs

  ## Summary
  Renames the row_type value 'upcoming' to 'nyp' (Not Yet Played) throughout both
  stat board RPCs. This aligns with the frontend enum StatHistoryStatus and gives
  a clearer semantic meaning:
  - 'nyp' = team has a scheduled fixture; game has NOT been played yet (NS status)
  - 'dnp' = team's game IS finished (FT) but player has no stat row
  - 'bye' = no fixture at all for the team that schedule week

  ## Changes
  - Rebuilds get_stat_board_players: timeline slot_type 'upcoming' → 'nyp'
  - Rebuilds get_stat_board_player_history: upcoming_rows row_type 'upcoming' → 'nyp'
  - No schema changes, no RLS changes, only string literal change
*/
...
-- ── Rebuild get_stat_board_players with 'nyp' ─────────────────────────────────
...
-- ── Rebuild get_stat_board_player_history with 'nyp' ─────────────────────────
...
-- NYP weeks = team has a scheduled game that is NOT finished (NS status)
nyp_weeks AS (
