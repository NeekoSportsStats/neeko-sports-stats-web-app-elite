/*
  # Fix populate_rankings_cache_from_source v2

  Changes from v1:
  - Removed ceiling_estimate and floor_estimate from INSERT (they are generated columns)
  - Write ceiling and floor directly instead
  - All other logic unchanged: no dependency on v_edge_board_core
*/
...
BEGIN
  ...
  -- ── Step 2: Upsert cache — no v_edge_board_core dependency ───────────────
  INSERT INTO afl.player_rankings_cache (
    ...
    summary_short, ai_summary, recommendation_color, recommendation_short,
    cached_at
  )
  SELECT
    pp.player_id,
    ...
    -- signal: pure percentile rank on edge — no view dependency
    CASE
      WHEN (
        COALESCE(prj.projection_final, pp.projection::numeric)
        - COALESCE(pp.last5_avg, pp.last3_avg, pp.season_avg, pp.projection)::numeric
    ...
    -- AI columns — NULL here, preserved via COALESCE on conflict
