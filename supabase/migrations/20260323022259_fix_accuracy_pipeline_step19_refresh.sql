/*
  # Fix Accuracy Freshness — Add Step 19 to run_neeko_pipeline

  ## Problem
  run_neeko_pipeline() (18 steps) never calls fn_refresh_player_accuracy_metrics().
  Steps 12-15 refresh the underlying projection_accuracy table and error tables,
  but afl.player_accuracy_metrics is never updated — it always shows stale timestamps.

  The views (v_projection_accuracy_homepage etc.) use MAX(created_at) from
  projection_accuracy which IS updated. But the accuracy diagnostics table
  (afl.player_accuracy_metrics) which AdminDataHub reads for accuracy_last_updated
  never gets refreshed.

  ## Fix
  Recreates run_neeko_pipeline() with one new final step (step 19):
    "Refresh Accuracy Metrics" — calls public.refresh_projection_accuracy()
    then fn_refresh_player_accuracy_metrics(p_season := 2026).

  The total_tasks count is updated from 18 to 19.

  ## Safety
  - All 18 existing steps are preserved exactly
  - Step 19 added at end (after log_model_accuracy_summary, before final status update)
  - Error in step 19 is non-fatal: pipeline still completes with partial status
*/
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, afl, ai, market, internal
AS $$
DECLARE
...
INSERT INTO public.pipeline_runs (
  ...
  19, 0, 'Starting', v_run_start, v_run_start
...
-- STEP 19 — NEW: Refresh Accuracy Metrics Table
-- Ensures afl.player_accuracy_metrics.updated_at is always fresh after pipeline run.
-- Also re-runs refresh_projection_accuracy() to catch any newly completed games.
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
