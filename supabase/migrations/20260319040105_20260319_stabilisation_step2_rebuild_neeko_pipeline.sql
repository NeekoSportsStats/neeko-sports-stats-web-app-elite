...
/*
  Stabilisation Step 2 — Rebuild run_neeko_pipeline()

  Changes vs previous version:
  1. Advisory lock (key=1) wraps the entire pipeline body. If a second
     caller arrives while the first is running it gets status=already_running
     immediately. Lock is always released in the outer EXCEPTION handler.
  2. Step 8 now calls afl.populate_rankings_cache_from_source() (canonical)
     instead of afl.refresh_player_rankings_cache_fast() (duplicate wrapper).
  3. Step 16 (NEW) — fn_refresh_edge_board() — fixes edge board staleness.
     Previously nothing in the automated pipeline called this, leaving the
     edge board table 2+ days stale.
  4. total_tasks updated from 15 to 16.
*/
...
BEGIN
  ...
  INSERT INTO public.pipeline_runs (
    ...
    16, 0, 'Starting', v_run_start, v_run_start
  ...
  -- STEP 8: populate rankings cache (canonical — populate_rankings_cache_from_source)
  v_step_start := clock_timestamp();
  BEGIN
    INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
  ...
  -- STEP 16: refresh edge board (NEW — was missing, caused 2+ day staleness)
  v_step_start := clock_timestamp();
  BEGIN
    INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
