/*
  # Fix populate_rankings_cache AI Schema Reference

  ## Critical Fix
  Corrects AI table schema reference from `afl.` to `public.` to resolve 
  "relation does not exist" error discovered during testing.

  ## Changes
  1. Fix AI table JOIN: `afl.ai_rankings_player_recos` → `public.ai_rankings_player_recos`
  
  This completes the fix started in previous migration which corrected the 
  `round_number` column error.
*/

-- ============================================================
-- Rebuild populate_rankings_cache with corrected AI schema
-- ============================================================
...
BEGIN
  ...
  LEFT JOIN public.ai_rankings_player_recos ai ON ai.player_id = nr.player_id  -- FIXED: public schema
...
'Populates player_rankings_cache from mv_player_rankings with pricing, AI data, and availability status. Fixes: column references and schema paths.';
