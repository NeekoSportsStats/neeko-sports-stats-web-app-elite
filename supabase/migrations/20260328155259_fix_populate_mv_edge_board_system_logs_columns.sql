/*
  # Fix populate_mv_edge_board — wrong system_logs column names

  ## Problem
  The function inserts into `system_logs` using non-existent columns:
  - `event_data` → should be `metadata`
  - `logged_at`  → should be `created_at`

  This caused step 16 of every run_neeko_pipeline() to fail with:
  "column event_data of relation system_logs does not exist"

  ## Fix
  Replace both INSERT statements (success log + exception handler) with
  correct column names matching the public.system_logs schema:
  (id, log_level, source, event_type, message, metadata, created_at)

  All other logic in the function is unchanged.
*/
...
DECLARE
v_inserted int := 0;
v_sections text[] := ARRAY['captain', 'breakout', 'trap'];
v_sec text;
BEGIN
-- Build all sections and upsert — never TRUNCATE first
-- This ensures no zero-row window for concurrent readers
...
(CASE WHEN COALESCE(consistency_score, 100) <= 50 THEN 1 ELSE 0 END) +
