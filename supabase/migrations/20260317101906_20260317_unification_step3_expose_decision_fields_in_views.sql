/*
  # NEEKO SPORTS — FULL SYSTEM UNIFICATION: Step 3
  Expose start_sit_decision, edge_score, edge_tier, market_watch_category in public views

  ## Summary
  Rebuilds v_rankings_master and v_rankings_free to include the four new derived
  decision columns. All downstream pages can now read a single consistent set of
  signals from these views without computing anything themselves.

  ## Changes
  - v_rankings_master: adds start_sit_decision, edge_score, edge_tier, market_watch_category, recommendation_strength, upside_pct
  - v_rankings_free: adds same columns (gated by row_rank thresholds)
*/
...
SELECT
  ...
  CASE WHEN row_rank <= 15 THEN recommendation_strength ELSE NULL::text END AS recommendation_strength,
  ...
  cached_at,
  row_rank,
  CASE WHEN row_rank <= 15 THEN start_sit_decision ELSE NULL::text END AS start_sit_decision,
  CASE WHEN row_rank <= 15 THEN edge_score ELSE NULL::integer END AS edge_score,
  CASE WHEN row_rank <= 15 THEN edge_tier ELSE NULL::text END AS edge_tier,
  CASE WHEN row_rank <= 15 THEN market_watch_category ELSE NULL::text END AS market_watch_category
FROM ranked
ORDER BY row_rank;
