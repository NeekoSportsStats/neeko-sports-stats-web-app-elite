/*
  # Enforce final availability logic on Edge Board

  ## Summary
  Redefines availability as: manual_status IS NULL AND is_bye = false.
  Players on bye or with any manual_status override are excluded from Edge Board
  captain/breakout/trap selections.

  ## Changes
  - Rebuilds `public.v_edge_board_safe` with strict availability filter:
    COALESCE(manual_status, '') = '' AND COALESCE(is_bye, false) = false
  - Also excludes projection = 0 / null players
*/
