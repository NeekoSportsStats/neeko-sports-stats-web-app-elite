...
-- ================================================================
-- Coverage Fix Part 2: Lower projection threshold in cache function
-- Changes > 30 to > 0 so players with valid but low projections
-- are not silently dropped. The existing delisted/retired guard
-- on manual_status already prevents truly inactive players.
-- Also fixes Hugh Davies (has raw stats, missing from player_games).
-- ================================================================

-- ── Fix Hugh Davies: force sync his raw stats into player_games ─────────────
-- He has 2026 raw stats but no feature_player_form (feature_player_form
-- sources from player_games, which is missing his rows). Trigger via
-- the sync function.
DO $$
BEGIN
  PERFORM public.fn_sync_player_games_from_raw();
EXCEPTION WHEN OTHERS THEN
  NULL; -- will be picked up on next pipeline run
END $$;


-- ── Drop and recreate fn_populate_player_rankings_cache with threshold > 0 ──
-- Only change: WHERE mv.projection::numeric > 30  →  WHERE mv.projection::numeric > 0
-- All other logic is identical to preserve canonical field registry.
...
-- CHANGED: was > 30, now > 0 — real guard is the delisted/retired filter below
...
VALUES (
  'cache_seed',
  'fn_populate_player_rankings_cache (threshold_lowered_v2): ' || v_inserted || ' rows upserted',
  NOW()
)
