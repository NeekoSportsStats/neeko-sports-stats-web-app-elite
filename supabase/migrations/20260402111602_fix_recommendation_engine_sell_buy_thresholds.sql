/*
  # Fix Recommendation Engine — Rebalance BUY / HOLD / SELL Distribution

  ## Summary
  The current SELL condition requires BOTH value_score <= -8 AND projection < breakeven
  simultaneously — a double-gate that produces too few SELL signals (~12%).
  
  Target distribution: BUY 8-12% | HOLD 60-70% | SELL 15-25%
  Simulated result with new thresholds: BUY 11.3% | HOLD 65.9% | SELL 22.8%

  ## Changes to `afl.populate_rankings_cache_from_source`

  ### SELL logic (any one condition triggers SELL):
  - Hard SELL: value_score <= -10  (clearly overpriced regardless of anything else)
  - Soft SELL: value_score <= -6   (meaningfully overpriced)
  - Combo SELL: value_score <= -4 AND projection < breakeven  (underperforming + below breakeven)

  ### BUY logic (both conditions required):
  - value_score >= 8 AND projection > breakeven + 3

  ### HOLD:
  - Everything else (true middle ground — no default bias)

  ## Changes to `public.get_rankings_updated_at`
  Falls back to `afl.player_rankings_cache.cached_at` when `ai_rankings_player_recos` is empty,
  fixing the "Last updated unknown" UI issue.
*/

-- ── Step 1: Rebuild populate_rankings_cache_from_source with new recommendation thresholds ──
...
BEGIN
  ...
  SELECT
    ...
    -- ── REBALANCED RECOMMENDATION ENGINE ─────────────────────────────────────
    -- SELL: any one of these conditions is sufficient
    --   Hard SELL: value_score <= -10 (clearly overpriced)
    --   Soft SELL: value_score <= -6  (meaningfully overpriced)
    --   Combo SELL: value_score <= -4 AND projection below breakeven
    -- BUY: value_score >= 8 AND projection clearly above breakeven
    -- HOLD: true middle ground — no default bias
    CASE
    ...
    CASE
      WHEN pp.value_score <= -10                                          THEN 'Overpriced — consider selling'
      WHEN pp.value_score <= -6                                           THEN 'Below value — trade target'
      WHEN pp.value_score <= -4 AND pp.projection < be.breakeven          THEN 'Not earning price — sell risk'
      WHEN pp.value_score >= 8  AND pp.projection > (be.breakeven + 3)   THEN 'Strong buy signal'
    ...
    CASE
      ...
      WHEN pp.value_score <= -4 AND pp.projection < be.breakeven          THEN 'Not scoring enough to maintain price — sell risk elevated'
    ...
    -- ── END REBALANCED RECOMMENDATION ENGINE ─────────────────────────────────
