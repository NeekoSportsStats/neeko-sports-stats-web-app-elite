/*
  # Fix free-tier value_score_canonical display in Edge Board and Market Watch

  ## Problem
  Both `get_edge_board_safe` and `get_market_watch_safe` were gating
  `value_score_canonical` behind premium/free_ids checks. This caused:
  - Edge Board free hero cards to show "—" instead of a real value score
  - Market Watch free rows to show "—" in the Value column

  ## Changes
  1. `get_edge_board_safe`: expose `value_score_canonical` for ALL rows
  2. `get_market_watch_safe`: expose `value_score_canonical` for premium + free_ids

  ## Security
  - AI text remains gated as before
  - Premium-only fields (edge_canonical, matchup_multiplier) unchanged
*/

-- ─── 1. Fix get_edge_board_safe ───────────────────────────────────────────────
...
BEGIN
  ...
  SELECT
    ...
    -- value_score_canonical: ALL rows (display metric, not premium content)
...
BEGIN
  ...
  SELECT
    ...
    CASE WHEN v_is_premium
         THEN c.edge_canonical ELSE NULL END,
    -- value_score_canonical: premium + free_ids (was: premium only)
    CASE WHEN v_is_premium OR c.player_id = ANY(v_free_ids)
