-- Extend get_social_planner_player_stats to include thresholds 10–14
-- in the all_threshold_hit_rates JSONB field.
-- The CROSS JOIN filter and primary threshold columns remain unchanged
-- (still 15/20/25/30 for disposal rows). Only the JSONB payload is extended.
...
season_stats AS (
  SELECT
    ...
    SUM(CASE WHEN pg.disposals >= 10 THEN 1 ELSE 0 END)                  AS ss_d10,
    SUM(CASE WHEN pg.disposals >= 11 THEN 1 ELSE 0 END)                  AS ss_d11,
    SUM(CASE WHEN pg.disposals >= 12 THEN 1 ELSE 0 END)                  AS ss_d12,
    SUM(CASE WHEN pg.disposals >= 13 THEN 1 ELSE 0 END)                  AS ss_d13,
    SUM(CASE WHEN pg.disposals >= 14 THEN 1 ELSE 0 END)                  AS ss_d14,
...
combined AS (
  SELECT
    ...
    ss.ss_d10, ss.ss_d11, ss.ss_d12, ss.ss_d13, ss.ss_d14,
...
disposal_rows AS (
  SELECT
    ...
    jsonb_build_object(
      '10', jsonb_build_object('hits', c.ss_d10, 'games', c.c_games_played, 'rate', ROUND(c.ss_d10::numeric / NULLIF(c.c_games_played, 0), 4)),
      '11', jsonb_build_object('hits', c.ss_d11, 'games', c.c_games_played, 'rate', ROUND(c.ss_d11::numeric / NULLIF(c.c_games_played, 0), 4)),
      '12', jsonb_build_object('hits', c.ss_d12, 'games', c.c_games_played, 'rate', ROUND(c.ss_d12::numeric / NULLIF(c.c_games_played, 0), 4)),
      '13', jsonb_build_object('hits', c.ss_d13, 'games', c.c_games_played, 'rate', ROUND(c.ss_d13::numeric / NULLIF(c.c_games_played, 0), 4)),
      '14', jsonb_build_object('hits', c.ss_d14, 'games', c.c_games_played, 'rate', ROUND(c.ss_d14::numeric / NULLIF(c.c_games_played, 0), 4)),
