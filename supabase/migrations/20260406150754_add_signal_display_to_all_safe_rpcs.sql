/*
  # Add signal_display to All Safe RPCs

  ## Purpose
  Adds the new signal_display column (e.g. '🔥 Target', 'Target', 'Watch', 'Avoid', '🚫 Hard Avoid')
  to all 4 public safe RPCs so the frontend can display rich signal labels without
  needing to derive them client-side.

  ## RPCs Updated
  1. get_rankings_safe — adds signal_display column
  2. get_market_watch_safe — adds signal_display column
  3. get_edge_board_safe — adds signal_display column
  4. get_player_detail_safe — adds signal_display column

  ## Notes
  - signal_display is always returned (not gated behind premium)
  - category column already shows Target/Watch/Avoid — signal_display adds emoji + strong tier labels
  - STRONG_BUY gets '🔥 Target', STRONG_SELL gets '🚫 Hard Avoid'
*/
