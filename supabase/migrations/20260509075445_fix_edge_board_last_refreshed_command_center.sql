/*
  # Fix edge_board_last_refreshed in v_command_center_status

  ## Problem
  v_command_center_status returns NULL::timestamp for edge_board_last_refreshed.
  admin.v_system_state never exposed a timestamp for the edge board.

  ## Fix
  Drop and recreate both views (required because CREATE OR REPLACE cannot rename
  existing view columns). All column names preserved exactly as they were.
  Only change: edge_board_last_refreshed now returns the real MAX(cached_at)
  from afl.player_rankings_cache instead of NULL.
*/
...
SELECT
  ...
  queue_failed,
  CASE
    WHEN queue_failed > 20 THEN 'error'
    WHEN queue_failed > 5  THEN 'warn'
    ELSE 'ok'
  END                                                          AS queue_health,
  mw_last_snapshot                                             AS market_watch_last_refresh,
  CASE
    WHEN mw_player_count >= 400 THEN 'ok'
    WHEN mw_player_count >= 100 THEN 'warn'
    ELSE 'error'
  END                                                          AS market_watch_quality,
  CASE
    WHEN mw_player_count >= 200 THEN 'ok'
    WHEN mw_player_count >= 50  THEN 'warn'
    ELSE 'error'
  ...
  CASE
    WHEN errors_24h > 50 THEN 'error'
    WHEN errors_24h > 10 THEN 'warn'
    ELSE 'ok'
  END                                                          AS logs_health,
  ...
  -- Fixed: was NULL::timestamp — now the real edge board refresh timestamp
  edge_board_refreshed_at                                      AS edge_board_last_refreshed,
  edge_board_rows
