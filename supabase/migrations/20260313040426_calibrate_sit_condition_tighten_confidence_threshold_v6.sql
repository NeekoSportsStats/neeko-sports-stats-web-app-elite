/*
  # Calibrate SIT condition — tighten confidence threshold v6

  ## Summary
  SIT was at 40.8% because confidence < 60 alone was catching ~196 players.
  Tightens the SIT rule so that:
  - Low confidence alone (< 55) still triggers SIT (clear uncertainty)
  - High risk alone (≥ 75) still triggers SIT (clear danger)
  - The middle band (confidence 55–60) only triggers SIT when ALSO combined with elevated risk (≥ 55)
