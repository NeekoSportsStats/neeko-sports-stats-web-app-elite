/*
  # Fix Populate Rankings Cache - Final Working Version
  
  ## Problem
  - afl.players has: player_id, player_name, position_group, active, manual_status
  - afl.player_games has: player_id, team_name, team_id (no position)
  - Must use position_group for both position and position_group
  
  ## Solution
  Use only fields that actually exist in source tables.
*/
...
BEGIN
  ...
  SELECT
    ...
    -- Comprehensive is_available (CRITICAL: exclude byes)
    (
  ...
  LEFT JOIN player_teams pt ON pt.player_id = pp.player_id
  LEFT JOIN afl.teams t ON t.team_name = pt.team_name
  LEFT JOIN afl.team_byes tb ON tb.team_id = COALESCE(pt.team_id, t.team_id) AND tb.season = 2026
  ...
  WHERE pp.player_id IS NOT NULL
    -- CRITICAL: Filter RETIRED/inactive/BYE at source
