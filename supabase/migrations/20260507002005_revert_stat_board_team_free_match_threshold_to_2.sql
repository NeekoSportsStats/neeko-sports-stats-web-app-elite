/*
  # Revert Team Stat Board Free Match Threshold from 4 to 2

  ## Summary
  Reverts the free preview threshold for team stat board RPCs from 4 matches back to 2.
  Only team RPCs are changed — get_stat_board_matches (players page) stays at <= 4.

  ## Affected Functions
  - get_stat_board_team_matches — team page match selector (is_free_match: <= 2, is_locked: > 2)
  - get_stat_board_team_rows   — team page data rows (is_free_match: <= 2, is_locked: > 2)

  ## Notes
  - get_stat_board_matches (players page) is NOT changed.
  - All other logic is preserved exactly; only the threshold values change.
*/
