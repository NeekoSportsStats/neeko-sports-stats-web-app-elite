/*
  # Fix 03: Create market.build_market_watch_snapshot function

  ## Problem
  The market.build_market_watch_snapshot() function does not exist (market schema
  was missing). The public.fn_process_market_watch_refresh_queue() calls this function
  and was silently failing.

  ## Solution
  Create the function. It sources all data from afl.player_rankings_cache enriched
  with afl.player_games last-3 average. Writes to all 4 market tables.

  ## Notes
  - Function is SECURITY DEFINER so it can write to market schema tables
  - search_path set to market, public, afl to resolve table references correctly
*/
