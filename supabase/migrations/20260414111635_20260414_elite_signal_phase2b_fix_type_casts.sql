/*
  # Elite Signal System — Phase 2b: Fix type casts in fn_populate_player_rankings_cache

  Fixes:
  - ceiling and floor are integer in mv_player_projection, need ::numeric cast for ROUND()
  - consistency, form_score need explicit ::numeric casts
  - neeko_rating needs ::numeric cast
  - confidence needs ::numeric cast
*/
...
pop_stats AS (
  SELECT
    ...
    AVG(LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric))         AS value_mean,
    NULLIF(STDDEV_POP(LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric)), 0) AS value_std
  FROM src_edge
),
  ...
  SELECT
    ...
    GREATEST(-3.0::numeric, LEAST(3.0::numeric,
      CASE WHEN ps.value_std IS NOT NULL
        THEN (LEAST(GREATEST(se.edge_raw, -40.0::numeric), 40.0::numeric) - ps.value_mean) / ps.value_std
        ELSE 0.0::numeric END
    )) AS value_z
  FROM src_edge se
...
src_with_confidence AS (
  SELECT *,
    ROUND(
      ...
      END::numeric * 0.20)
      -- role_stability 20%
      + (COALESCE(stability_score, 50.0::numeric) * 0.20)
      -- projection_stability 20%
      + (
          CASE
      ...
      + (COALESCE(base_confidence_score, 50.0::numeric) * 0.15)
      -- form_stability 15%
      + (
          CASE
            WHEN last_3_avg IS NULL OR season_avg IS NULL OR season_avg = 0 THEN 50.0
            WHEN ABS(last_3_avg - season_avg) / GREATEST(season_avg, 1::numeric) <= 0.08 THEN 88.0
            WHEN ABS(last_3_avg - season_avg) / GREATEST(season_avg, 1::numeric) <= 0.15 THEN 74.0
            WHEN ABS(last_3_avg - season_avg) / GREATEST(season_avg, 1::numeric) <= 0.25 THEN 58.0
            WHEN ABS(last_3_avg - season_avg) / GREATEST(season_avg, 1::numeric) <= 0.40 THEN 42.0
            ELSE 25.0
          END::numeric * 0.15
        )
      -- matchup_reliability 10%
      + (
          CASE
...
src_with_conf_pct AS (
  SELECT *,
    ...
    ROUND(
      100.0 * PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(conf_score_raw, 0.0::numeric), 100.0::numeric))
    , 1) AS conf_pct_val
...
src_with_conf_label AS (
  SELECT *,
    CASE
      WHEN conf_pct_val >= 82.0 THEN 'HIGH'
      WHEN conf_pct_val >= 30.0 THEN 'MEDIUM'
      ELSE                           'LOW'
...
src_with_value_band AS (
  SELECT *,
    ROUND(
      100.0 * PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric))
    , 1) AS edge_pct_val,
    CASE
      WHEN PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric)) >= 0.85
        THEN 'Elite Value'
      WHEN PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric)) >= 0.65
        THEN 'Strong Value'
      WHEN PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric)) >= 0.35
        THEN 'Fair Value'
      WHEN PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric)) >= 0.15
        THEN 'Thin Value'
      ELSE
        'Poor Value'
