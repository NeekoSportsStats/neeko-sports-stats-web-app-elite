/*
  # Rebuild get_player_detail_safe with canonical fields

  ## Summary
  Rebuilds the `get_player_detail_safe` RPC to include all canonical fields
  from `afl.player_rankings_cache` that the player page needs.

  ## Changes
  - Drops and recreates `public.get_player_detail_safe`
  - Adds: `edge_canonical`, `breakeven_canonical`, `signal_canonical`,
    `category_canonical`, `action_canonical`, `value_score_canonical`
  - Adds: `signal`, `trend_signal`, `form_label`, `form_delta`,
    `season_avg`, `last_3_avg`, `risk_rating`, `matchup_rating`,
    `captain_rating`, `captain_score`, `consistency_score`
  - Adds: `summary_short`, `summary_long`, `recommendation_short`
  - Preserves all existing fields and premium gating logic

  ## Notes
  - SECURITY DEFINER ensures access control is enforced server-side
  - Free users get basic fields only (is_locked=true gates premium fields)
  - Premium users (via stripe or manual_premium) get all fields
*/
...
RETURNS TABLE (
  ...
  value_tag                text,
  value_tier               text,
  -- legacy signal (keep for backwards compat)
  signal                   text,
  signal_tag               text,
...
BEGIN
  ...
  SELECT
    ...
    c.upside_rating,
    -- ai text
    CASE WHEN v_is_premium OR v_is_admin THEN c.summary_short        ELSE NULL END,
