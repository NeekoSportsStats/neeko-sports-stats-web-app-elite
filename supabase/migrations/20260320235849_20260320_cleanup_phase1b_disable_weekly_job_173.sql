/*
  # Phase 1B: Add model performance logging to stage3 and disable redundant weekly job 173

  ## Problem
  Job 173 (weekly_model_improvement_tuesday_melb) runs every Monday and executes:
    1. snapshot_player_projections_for_next_round()
    2. refresh_player_projection_error()
    3. refresh_projection_model_calibration()
    4. refresh_player_projection_confidence_calibrated()
    5. Logs MAE, RMSE, bias summary to system_logs

  Steps 1-4 are already executed nightly in stage3 (run_neeko_pipeline steps 11-14).
  The ONLY unique value in job 173 is the weekly MAE/RMSE/bias summary log in step 5.

  ## Solution
  1. Create fn_log_model_accuracy_summary() — extracts the logging step from job 173
  2. Add it as step 18 to run_neeko_pipeline (so nightly runs also log metrics)
  3. Disable/remove job 173 (no longer needed; stage3 now covers everything)

  ## Why safe
  - Steps 1-4 already run nightly — zero functionality lost
  - MAE/RMSE logging is preserved via new step 18 in stage3
  - Removing the Monday-only duplicate saves 4x calibration reruns per week
*/

-- Step A: Create standalone model accuracy logging function
CREATE OR REPLACE FUNCTION public.fn_log_model_accuracy_summary()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  SELECT
    ...
    COUNT(*)::integer
  INTO v_overall_mae, v_overall_rmse, v_overall_bias, v_error_count
  ...
  VALUES (
    ...
    'pipeline:model_accuracy',
    'info',
    'Model accuracy — MAE=' || COALESCE(v_overall_mae::text, 'n/a')
      || ' RMSE=' || COALESCE(v_overall_rmse::text, 'n/a')
      || ' bias=' || COALESCE(v_overall_bias::text, 'n/a')
      || ' samples=' || COALESCE(v_error_count::text, '0'),
    jsonb_build_object(
...
-- Step B: Add step 18 (model accuracy logging) to run_neeko_pipeline
-- Full replacement of run_neeko_pipeline adding step 18 at the end
CREATE OR REPLACE FUNCTION public.run_neeko_pipeline()
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public, afl, market, internal
AS $$
DECLARE
...
-- STEP 10: build market watch snapshot
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- STEP 18 (NEW): log model accuracy summary — MAE/RMSE/bias
-- Previously only ran weekly via job 173. Now runs nightly so accuracy is always current.
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- Step C: Disable job 173 (weekly_model_improvement_tuesday_melb)
-- Stage3 now handles all 4 calibration steps nightly AND logs MAE/RMSE via step 18
SELECT cron.unschedule('weekly_model_improvement_tuesday_melb');
