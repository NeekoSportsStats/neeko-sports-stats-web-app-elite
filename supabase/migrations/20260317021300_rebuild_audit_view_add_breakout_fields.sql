...
/*
  # Rebuild afl.v_projection_engine_audit — add breakout_probability and breakout_flag

  ## Summary
  Extends the audit view with two new diagnostic columns:
  - breakout_probability  — 0-1 score from player_breakout_model
  - breakout_flag         — TRUE when probability > 0.65

  All existing columns preserved including the venue matchup columns added
  in the previous migration.

  ## Notes
  - DROP then CREATE required (column set change)
*/
...
SELECT
  ...
  -- Breakout model
  ROUND(COALESCE(bm.breakout_probability, 0.0), 4)                               AS breakout_probability,
