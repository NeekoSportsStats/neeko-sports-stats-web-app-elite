/*
  # Fix Market Watch - Use Projection for last3_avg
  
  player_rankings_cache doesn't have last3_avg column
  Use projection_final instead
*/
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(rc.price, 0)::numeric,
    COALESCE(rc.prev_price, rc.price, 0)::integer,
    COALESCE(rc.price_change_pct, 0)::numeric,
    
    COALESCE(rc.projection_final, rc.projection, 0)::numeric,
    ROUND(COALESCE(rc.price, 0)::numeric / 7200.0, 1),
    COALESCE(rc.ceiling, rc.ceiling_estimate, rc.projection_final, 0)::numeric,
    COALESCE(rc.risk_rating, 50)::numeric,
    
    ROUND((COALESCE(rc.value_score, 0)::numeric - 100), 1),
    ROUND(((COALESCE(rc.projection_final, 0)::numeric - (COALESCE(rc.price, 0)::numeric / 7200.0)) * 800)::numeric, 0),
    
    -- Category from rankings cache or fallback logic
    CASE
      WHEN COALESCE(rc.value_score, 0) = 0 THEN 'monitor'
      WHEN rc.market_watch_category IS NOT NULL THEN LOWER(REPLACE(rc.market_watch_category, ' ', '_'))
    ...
    -- Action from ai_recommendation or value-based
    CASE
    ...
    ROUND(rc.price::numeric * 1.05),
    ROUND(rc.price::numeric * 1.03),
    ROUND(rc.price::numeric * 1.05),
    ROUND(rc.price::numeric * 1.08),
    
    GREATEST(0, ROUND((COALESCE(rc.value_score, 0)::numeric - 80), 1)),
    (rc.value_score > v_vs_p90 AND rc.neeko_rating > v_nr_p85),
    COALESCE(rc.risk_rating, 50)::numeric,
    CASE 
    ...
    -- Use projection_final for last3_avg (column doesn't exist in rankings_cache)
    COALESCE(rc.projection_final, 0)::numeric,
    rc.price::numeric,
    COALESCE(rc.value_score, 0)::numeric,
    
    ROUND(rc.price::numeric * 1.10),
    ROUND(rc.price::numeric * 0.92),
    ROUND((COALESCE(rc.value_score, 0)::numeric - 100), 1),
    CASE 
