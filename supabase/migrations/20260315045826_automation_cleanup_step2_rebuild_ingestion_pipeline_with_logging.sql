/*
  # Automation Cleanup — Step 2: Rebuild Ingestion Pipeline with Logging

  ## Summary
  Rebuilds `run_afl_worker_ingestion` to write proper pipeline_runs and
  pipeline_steps records. Previously this function fired HTTP POSTs with no
  observability — failures were invisible.

  ## Changes
  - Creates a pipeline_runs record (key: afl_ingestion) at start
  - Records each HTTP dispatch as a pipeline_step with timing
  - Marks run complete / error at end
  - Logs to system_logs on completion
  - Ingestion-only: does NOT trigger rankings cache, market watch, or AI

  ## Ingestion Steps (in order)
  1. ingest_teams   — afl-teams-worker
  2. ingest_games   — afl-worker-games
  3. ingest_players — afl-worker-players
  4. ingest_player_stats — afl-worker-games-player-stats
*/
