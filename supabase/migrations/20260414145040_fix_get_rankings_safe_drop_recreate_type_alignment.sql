/*
  # Fix get_rankings_safe — drop and recreate with correct types

  ## Problem
  Return signature declared columns as `double precision` but the view
  `afl.v_rankings_unified` returns them as `numeric`, causing:
  "returned type numeric does not match expected type double precision"

  ## Fix
  Drop and recreate with all numeric columns correctly typed as `numeric`,
  `price`/`prev_price`/`price_change` as `numeric`, `total_count` as `bigint`,
  `games_played` and `bye_round` as `numeric` — matching the view exactly.
*/
...
BEGIN
  ...
  SELECT
    ...
    c.price_change,
    c.price_change_pct,
    CASE WHEN v_is_premium OR c.player_id::int = ANY(v_free_ids) THEN c.breakeven              ELSE NULL::numeric END,
    CASE WHEN v_is_premium OR c.player_id::int = ANY(v_free_ids) THEN c.value_score            ELSE NULL::numeric END,
    CASE WHEN v_is_premium                                        THEN c.edge                  ELSE NULL::numeric END,
    ...
    CASE WHEN v_is_premium OR c.player_id::int = ANY(v_free_ids) THEN c.decision_score         ELSE NULL::numeric END,
    CASE WHEN v_is_premium OR c.player_id::int = ANY(v_free_ids) THEN c.confidence_score_100   ELSE NULL::numeric END,
    CASE WHEN v_is_premium OR c.player_id::int = ANY(v_free_ids) THEN c.confidence_percentile  ELSE NULL::numeric END,
