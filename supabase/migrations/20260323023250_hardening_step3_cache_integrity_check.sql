/*
  # Hardening Step 3 — Cache Integrity Check

  ## Summary
  Creates `afl.fn_cache_integrity_check()` — runs after every
  `populate_rankings_cache_from_source()` call and detects rows in
  `afl.player_rankings_cache` where `summary_short` or `summary_long` is NULL
  despite having valid data in `ai.player_ai_analysis`. Re-syncs those rows
  immediately so the cache is always consistent.

  ## What it does
  1. Finds rows in player_rankings_cache where summary_short/summary_long IS NULL
     but ai.player_ai_analysis has non-NULL values for the same player.
  2. Re-syncs those columns from the canonical AI table.
  3. Logs the count of rows repaired to system_logs.
  4. Returns the count of repaired rows.

  ## Where it runs
  Called directly inside `afl.populate_rankings_cache_from_source()` at the
  end (added via a separate pipeline rebuild in step 4 migration).
*/

CREATE OR REPLACE FUNCTION afl.fn_cache_integrity_check()
RETURNS integer
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SET
    summary_short        = aia.summary_short,
    summary_long         = aia.summary_long,
    recommendation_short = COALESCE(c.recommendation_short, aia.summary_short),
    recommendation_why   = COALESCE(c.recommendation_why,   aia.summary_short),
    ai_summary           = COALESCE(c.ai_summary,           aia.summary_long),
    ai_prompt_version    = COALESCE(c.ai_prompt_version,    aia.model),
    ai_generated_at      = COALESCE(c.ai_generated_at,      aia.generated_at),
    ai_updated_at        = COALESCE(c.ai_updated_at,        aia.generated_at),
    ai_validation_passed = CASE
      WHEN aia.generated_at IS NOT NULL AND aia.summary_short IS NOT NULL
      THEN true ELSE false END,
    cached_at            = now()
  FROM ai.player_ai_analysis aia
  WHERE aia.player_id = c.player_id
    AND aia.summary_short IS NOT NULL
    AND aia.summary_long  IS NOT NULL
  ...
  IF v_repaired > 0 THEN
    ...
    VALUES (
      'cache_integrity_repaired', 'afl.fn_cache_integrity_check', 'warn',
      'Cache integrity check: re-synced ' || v_repaired || ' rows missing AI content',
      jsonb_build_object('repaired_count', v_repaired, 'repaired_at', now())
    );
  END IF;
...
GRANT EXECUTE ON FUNCTION afl.fn_cache_integrity_check() TO service_role;

-- ── Wire fn_cache_integrity_check into populate_rankings_cache_from_source ──
-- We add the call at the end of the function, just before RETURN v_count.
-- Re-create with the integrity check added after fn_rebuild_confidence_scores().
...
-- ── STEP 3: Cache integrity check — re-sync any rows where AI content is missing ──
