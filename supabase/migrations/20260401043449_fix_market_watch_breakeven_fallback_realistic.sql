/*
  # Fix Market Watch Breakeven Fallback - Use Realistic Estimate
  
  ## Summary
  Fixes the breakeven fallback for players without cached breakeven values.
  33% of players have NULL breakeven, causing impossible values (300-400).
  
  ## Problem
  Current fallback: `price / 2500` produces impossible values
  - Adam Treloar: price $946k → breakeven 378.4 (IMPOSSIBLE)
  - Zac Fisher: price $787k → breakeven 314.8 (IMPOSSIBLE)
  - Taylor Adams: price $581k → breakeven 232.4 (IMPOSSIBLE)
  
  ## Solution
  Use projection as breakeven estimate when NULL
  - Adam Treloar: projection 66.81 → breakeven 66.81 (REALISTIC)
  - Zac Fisher: projection 46.23 → breakeven 46.23 (REALISTIC)
  - Taylor Adams: projection 35.37 → breakeven 35.37 (REALISTIC)
  
  ## Logic
  If breakeven is NULL, assume player needs to match their projection to hold price.
  This is a reasonable estimate when we don't have breakeven data.
  
  ## Impact
  201 players (33.4%) affected by this change
*/
...
BEGIN
  ...
  WITH ranked_players AS (
    SELECT
      ...
      -- FIXED: Use projection as realistic fallback when breakeven is NULL
      -- Assumes player needs to match projection to hold price (reasonable estimate)
      COALESCE(
  ...
  SELECT
    ...
    -- vs BE = projection - breakeven (now with correct breakeven for all players)
...
-- Rebuild snapshot with realistic breakeven fallback
