/*
  # Backend Lock Step 2 — Fix populate_rankings_cache_from_source

  ## Summary
  Rewrites the cache populate function so that on every future pipeline run,
  edge_tier and market_watch_category are written correctly from the canonical
  signal field — not from legacy price-based logic.

  ## Key Fix
  - edge_tier now = signal_val (was: AVOID/ELITE/NEUTRAL/STRONG/WEAK)
  - market_watch_category now = signal-derived TARGET/WATCH/AVOID (consistent with signal_tag)
  - signal_tag still = TARGET/WATCH/AVOID (unchanged, correct)
...
BEGIN
  ...
  SELECT
    ...
    -- edge_tier now = signal (canonical, single source of truth)
    ...
    -- signal_tag = canonical 3-tier label
    CASE
      WHEN s.signal_val IN ('STRONG_BUY', 'BUY') THEN 'TARGET'
      WHEN s.signal_val IN ('SELL', 'STRONG_SELL') THEN 'AVOID'
      ELSE 'WATCH'
    END                                           AS signal_tag,
    -- market_watch_category always equals signal_tag (no more divergence)
    CASE
      WHEN s.signal_val IN ('STRONG_BUY', 'BUY') THEN 'TARGET'
      WHEN s.signal_val IN ('SELL', 'STRONG_SELL') THEN 'AVOID'
      ELSE 'WATCH'
