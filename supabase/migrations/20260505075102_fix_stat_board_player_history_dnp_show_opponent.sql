/*
  # Fix get_stat_board_player_history — show opponent for DNP rows

  DNP rows now correctly derive the opponent team name and home/away indicator
  from team_weeks, so the game log can display which game the player missed.
*/
...
team_weeks AS (
  SELECT
    ...
    g.away_team_id,
    g.away_team_name,
    -- Opponent and home/away from the team's perspective
    CASE
      WHEN pt.team_id = g.home_team_id THEN g.away_team_name
      ELSE g.home_team_name
    END AS opponent_name,
    (pt.team_id = g.home_team_id) AS team_is_home
  FROM afl.games g
...
-- Weeks in the completed schedule where no team game exists (true bye weeks)
...
played AS (
  ...
  WHERE pg.player_id = p_player_id
    AND pg.season    = p_season
    -- Exclude all-zero/corrupt rows (true DNP stored as zeros)
...
dnp_rows AS (
  SELECT
    ...
    tw.opponent_name    AS opponent_team_name,
    tw.venue            AS venue,
    tw.team_is_home     AS is_home,
    NULL::integer       AS disposals,
    NULL::integer       AS kicks,
    NULL::integer       AS handballs,
    NULL::integer       AS marks,
