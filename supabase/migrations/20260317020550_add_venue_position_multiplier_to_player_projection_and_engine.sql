...
/*
  # Add venue_position_multiplier to player_projection and integrate into engine

  ## Summary
  1. Adds new column `venue_position_multiplier` to afl.player_projection
  2. Backfills existing rows
  3. Updates afl.refresh_projection_engine() to refresh venue concession and
     populate venue_position_multiplier per player before the MV refresh

  ## Integration
  final_matchup = matchup_rating * position_concession_multiplier * venue_position_multiplier
  (the new column multiplies on top of the existing two matchup signals)

  ## Notes
  - venue_position_multiplier defaults to 1.0 (no effect) when sparse
  - Additive only — no existing columns altered
*/

-- Step 1: add column to player_projection
DO $$
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'afl'
      ...
      AND column_name  = 'venue_position_multiplier'
  ) THEN
    ALTER TABLE afl.player_projection
      ADD COLUMN venue_position_multiplier numeric(6,4) NOT NULL DEFAULT 1.0;
  END IF;
END $$;
...
-- Step 3: rebuild refresh_projection_engine
...
BEGIN
  ...
  -- Step 3: feature_player_form (role-change-aware weighting)
  ...
  -- Step 5: confidence refresh
  SELECT afl.refresh_player_projection_confidence() INTO v_conf;

  -- Step 6: populate venue_position_multiplier on player_projection
  UPDATE afl.player_projection pp
  SET venue_position_multiplier = COALESCE(opvc.concession_multiplier, 1.0)
  FROM afl.players               pl
  JOIN afl.v_current_player_team cpt ON cpt.player_id = pl.player_id
