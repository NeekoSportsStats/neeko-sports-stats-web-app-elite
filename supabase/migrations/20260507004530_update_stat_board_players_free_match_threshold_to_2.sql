/*
  # Update Player Stat Board Free Match Threshold from 4 to 2

  ## Summary
  Reduces the free preview limit for the Player Stat Board from 4 matches to 2 matches.
  Affects `get_stat_board_matches` (match selector) and `get_stat_board_players` (player data rows).
  Team Stat Board RPCs (get_stat_board_team_matches, get_stat_board_team_rows) are NOT changed —
  they remain at 2 and are already correct.

  ## Changes
  1. `get_stat_board_matches` — is_free_match: (match_order <= 2), is_locked: (match_order > 2)
  2. `get_stat_board_players` — active_fixture CTE: (rf.match_order <= 2) AS is_free_match

  ## Notes
  - Premium bypass (isPremium || isAdmin) remains entirely client-side — unchanged.
  - Fixture order is determined by game_date ASC, game_id ASC within the round.
  - Return types are preserved exactly; only threshold values change.
*/

-- ── get_stat_board_matches: threshold 4 → 2 ──────────────────────────────────
...
-- ── get_stat_board_players: active_fixture threshold 4 → 2 ───────────────────
