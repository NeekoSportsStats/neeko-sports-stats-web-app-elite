/*
  # Calibrate action thresholds based on actual data distribution

  ## Changes
  - SMASH_START: >= 0.80 (was 1.15) → targets ~12% of players
  - START:       >= 0.38 (was 0.45) → targets ~16% of players  
  - HOLD:        > -0.35 (unchanged) → targets ~43% of players
  - SIT:         > -0.90 (was -1.00) → targets ~18% of players
  - HARD_SIT:    <= -0.90 (was -1.00) → targets ~10% of players

  Distribution target: SMASH_START 8-12%, START 18-25%, HOLD 35-45%, SIT 15-22%, HARD_SIT 8-12%
*/
...
VALUES (
  'cache_seed',
  'fn_populate_player_rankings_cache (elite_signal_v1_calibrated): ' || v_inserted || ' rows upserted',
  NOW()
)
