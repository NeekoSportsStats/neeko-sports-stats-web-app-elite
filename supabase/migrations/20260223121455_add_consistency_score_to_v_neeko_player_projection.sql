/*
  # Add consistency_score to afl.v_neeko_player_projection

  ## Change
  - Replaces the view with an identical definition plus a new `consistency_score` column
  - consistency_score = ROUND(100 * PERCENT_RANK() OVER (ORDER BY volatility_last_15 ASC))
  - Lower volatility → higher percentile → higher consistency score (0–100)

  ## Notes
  - PERCENT_RANK is a window function applied across all players in the final SELECT
  - Scores are rounded to the nearest integer for clean display
*/
...
WITH schedule AS (
    ...
    SELECT v_neeko_player_recent_games.player_id,
           round(avg(CASE WHEN v_neeko_player_recent_games.row_num <= 3  THEN v_neeko_player_recent_games.fantasy_points ELSE NULL::integer END), 2) AS avg_last_3,
           round(avg(CASE WHEN v_neeko_player_recent_games.row_num <= 5  THEN v_neeko_player_recent_games.fantasy_points ELSE NULL::integer END), 2) AS avg_last_5,
           round(avg(CASE WHEN v_neeko_player_recent_games.row_num <= 10 THEN v_neeko_player_recent_games.fantasy_points ELSE NULL::integer END), 2) AS avg_last_10,
           round(avg(CASE WHEN v_neeko_player_recent_games.row_num <= 15 THEN v_neeko_player_recent_games.fantasy_points ELSE NULL::integer END), 2) AS avg_last_15,
           round(stddev_pop(CASE WHEN v_neeko_player_recent_games.row_num <= 15 THEN v_neeko_player_recent_games.fantasy_points ELSE NULL::integer END), 2) AS volatility_last_15,
           round(percentile_cont(0.10::double precision) WITHIN GROUP (ORDER BY (CASE WHEN v_neeko_player_recent_games.row_num <= 15 THEN v_neeko_player_recent_games.fantasy_points ELSE NULL::integer END::double precision))::numeric, 1) AS floor_estimate,
           round(percentile_cont(0.90::double precision) WITHIN GROUP (ORDER BY (CASE WHEN v_neeko_player_recent_games.row_num <= 15 THEN v_neeko_player_recent_games.fantasy_points ELSE NULL::integer END::double precision))::numeric, 1) AS ceiling_estimate,
    ...
    SELECT ps.player_id,
           ...
           CASE
               ...
               WHEN ps.games_played_2026 >= 1 AND ps.games_played_2026 <= 5 THEN 'EARLY_2026_BLENDED'
               WHEN ps.games_played_2026 >= 6 AND ps.games_played_2026 <= 10 THEN 'MID_2026_BLENDED'
...
SELECT
    ...
    f.is_home,
    f.match_date,
    COALESCE(f.round_number, (SELECT next_round_num.rn FROM next_round_num)) AS target_round_number,
    b.season_context,
    b.games_played_2025,
    b.baseline_avg_2025,
    b.games_played_2026,
    ...
    b.final_projection,
    ROUND(
        100 * PERCENT_RANK() OVER (ORDER BY b.volatility_last_15 ASC)
