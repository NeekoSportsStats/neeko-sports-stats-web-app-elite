/*
  # Rebuild get_stat_board_players() — Fixture-Based Freemium v2

  ## Problem Fixed
  Previous version incorrectly derived is_free_match from the player's most recent
  historical game joined to round_match_order. This caused players whose last game
  was not in the selected round to default to is_free_match=true.

  ## Root Cause
  afl.players has no team_id column. Team assignment is only in afl.player_games.
  The fixture join must therefore:
  1. Derive each player's current team from their most recent player_games row.
  2. Join that current team to the selected fixture (home_team_id / away_team_id).
  3. Apply lock state from the fixture — not from the player's game history.

  ## Architecture
  - resolved_round: p_round or MAX(week) from afl.games
  - round_fixtures: all games for that week with match_order from game_date order
  - active_fixture: p_match_id if given, else match_order=1
  - player_current_team: DISTINCT ON player_id ordered by week DESC — most recent team
  - fixture_players: players whose current team matches either side of active_fixture
  - season_games / ranked_games / agg: historical rolling stats for those players only
  - Final SELECT: fixture fields + rolling stats merged

  ## Security
  SECURITY DEFINER, search_path = 'afl','public'
  GRANT EXECUTE to anon, authenticated
*/

DROP FUNCTION IF EXISTS public.get_stat_board_players(
  integer, integer, text, numeric, text, text, text, integer, integer
);
DROP FUNCTION IF EXISTS public.get_stat_board_players(
  integer, integer, integer, text, numeric, text, text, text, integer, integer
);
DROP FUNCTION IF EXISTS public.get_stat_board_players(
  integer, integer, integer, text, numeric, text, integer, text, integer, integer
);
...
resolved_round AS (
  SELECT COALESCE(
    p_round,
    (SELECT MAX(g.week) FROM afl.games g WHERE g.season = p_season)
  ) AS rnd
),
...
-- ── 4. Active fixture: p_match_id or match_order=1 ───────────────────────────
active_fixture AS (
  SELECT
    rf.game_id,
    rf.week,
...
-- ── 5. Each player's current team from most recent game in season ────────────
...
-- ── 7. Historical rolling stats — last 10 games in season up to selected round
season_games AS (
  SELECT
    pg.player_id AS pid,
    pg.week      AS wk,
