/*
  # Fix Match Centre RPC: Add p_is_premium parameter

  ## Problem
  The get_stat_board_match_centre_rows RPC hardcodes gating at fixture_order <= 2,
  regardless of the caller's subscription status. Premium users see null data for
  all fixtures beyond the first two even though they are entitled to full access.

  ## Fix
  Add p_is_premium boolean parameter (default false). When true, the "locked" check
  becomes always-false and all stat fields are returned for every fixture.

  ## Changes
  - New parameter: p_is_premium boolean DEFAULT false
  - All CASE WHEN mtp.mtp_order <= 2 guards replaced with:
      CASE WHEN (p_is_premium OR mtp.mtp_order <= 2)
  - is_locked column now returns false for premium users on all fixtures
  - is_premium_unlocked returns true for premium users on fixtures beyond the free limit
*/
...
SELECT
  ...
  -- Premium users never see a lock
  CASE WHEN p_is_premium THEN false ELSE (mtp.mtp_order > 2) END             AS is_locked,
  -- Flag fixtures unlocked by premium (beyond the free limit)
  CASE WHEN p_is_premium AND mtp.mtp_order > 2 THEN true ELSE false END      AS is_premium_unlocked,
  CASE WHEN (NOT p_is_premium) AND mtp.mtp_order > 2 THEN 'Unlock full round' ELSE NULL END AS lock_reason,
  ...
  -- Stat fields: gated by free preview OR premium access
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN COALESCE(ta.ta_recent_values, ARRAY[]::numeric[]) ELSE NULL END AS recent_values,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN COALESCE(ta.ta_games_count, 0) ELSE NULL END::integer           AS recent_games_count,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_avg_l3 ELSE NULL END                                      AS recent_avg_l3,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_avg_l5 ELSE NULL END                                      AS recent_avg_l5,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_avg_l8 ELSE NULL END                                      AS recent_avg_l8,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_season_avg ELSE NULL END                                  AS season_avg,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN oc.oc_l5 ELSE NULL END                                          AS opponent_conceded_l5,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN oc.oc_season ELSE NULL END                                      AS opponent_conceded_season,

  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN
    CASE
  ...
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_low ELSE NULL END     AS low_recent,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_high ELSE NULL END    AS high_recent,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_stddev ELSE NULL END  AS stddev_recent,
  ...
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) AND p_lens = 'score' THEN
  ...
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) AND p_lens IN ('score', 'goals', 'scoring_shots') THEN
    CASE
  ...
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_goals_avg ELSE NULL END    AS recent_goals_avg,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_behinds_avg ELSE NULL END  AS recent_behinds_avg,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_shots_avg ELSE NULL END    AS recent_scoring_shots_avg,

  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) AND ta.ta_shots_avg > 0
    THEN ROUND((ta.ta_goals_avg / ta.ta_shots_avg * 100)::numeric, 1)
    ELSE NULL END                                                            AS conversion_rate,

  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_opp_l5 ELSE NULL END      AS opponent_points_conceded_l5,
  CASE WHEN (p_is_premium OR mtp.mtp_order <= 2) THEN ta.ta_opp_season ELSE NULL END   AS opponent_points_conceded_season,
