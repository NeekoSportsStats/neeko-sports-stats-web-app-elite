/*
  # Fix populate_rankings_cache - Column Alignment v2

  ## Critical Fix
  Aligns function with actual mv_player_rankings columns.
  Derives missing fields from available data or uses sensible defaults.

  ## Changes
  1. Remove references to non-existent columns (value_tier, consistency_tier, etc.)
  2. Derive tier values from scores
  3. Use actual column names from mv_player_rankings
  4. Fix price column references
  5. Fix AI schema reference
*/
...
BEGIN
  ...
  SELECT
    ...
    -- Price columns with proper fallbacks
    ...
    -- Upside (derive from ceiling vs projection)
    CASE
    ...
    -- Captain score (use neeko_rating as proxy)
    round((
      (nr.projection::numeric                                          * 0.55) +
      (COALESCE(nr.confidence, 50.0)::numeric                         * 0.23) +
      (COALESCE(nr.consistency, 50.0)::numeric                        * 0.17) +
    ...
    -- Captain rating (derive from projection)
    CASE
  ...
  LEFT JOIN public.ai_rankings_player_recos ai ON ai.player_id = nr.player_id
...
'Populates player_rankings_cache from mv_player_rankings. Fixes: column references, schema paths, and derives missing tier values from scores.';
