/*
  # Fix mv_player_projection — Remove Stale COALESCE(value_score, 50.0) Fallback

  ## Problem
  mv_player_projection uses COALESCE(fp.value_score, 50.0) which:
  1. Treats NULL value_score (excluded players) as 50 — inflating their neeko_rating
  2. Was calibrated for the old [0, 14] scale where 50 was a neutral fallback
  3. On the new [-35, +48] scale, 50 is an extreme outlier, not neutral

  ## Fix
  - Replace COALESCE(fp.value_score, 50.0) with COALESCE(fp.value_score, 0.0)
  - 0.0 is now the natural neutral midpoint on the new scale
  - Applied in both the SELECT column and the neeko_rating formula
  - Refresh the materialized view after definition change
*/
...
SELECT
  ...
  -- value_score: use 0.0 as neutral fallback (midpoint of new [-35, +48] scale)
  COALESCE(fp.value_score, 0.0) AS value_score,
  -- neeko_rating: value_score now on new scale, 0.0 is neutral
  round(
    ...
    + COALESCE(fp.value_score, 0.0) * 0.15
