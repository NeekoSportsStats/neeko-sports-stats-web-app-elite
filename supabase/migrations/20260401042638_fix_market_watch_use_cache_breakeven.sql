/*
  # Fix Market Watch - Use Rankings Cache Breakeven
  
  ## Summary
  Fixes breakeven to use the already-calculated value from player_rankings_cache
  instead of recalculating incorrectly as price/2500.
  
  ## Problem
  Current: `ROUND((COALESCE(rc.price, 0)::numeric / 2500.0), 1) as breakeven`
  - Results in 400-480 for premium players (WRONG)
  - This is the price divided by 2500, not actual breakeven
  
  Correct: `rc.breakeven`
  - Already calculated correctly in rankings cache
  - Range: 60-120 (realistic AFL Fantasy breakevens)
  
  ## Examples
  Nick Daicos:
  - Price: $1,204,000
  - OLD breakeven: 1204000 / 2500 = 481.6 (IMPOSSIBLE)
  - NEW breakeven: 112.6 from cache (CORRECT)
  
  ## Changes
  Line 62: Change from calculated to cached breakeven
  Line 112: vs BE already correct (projection - breakeven)
*/
...
BEGIN
  ...
  WITH ranked_players AS (
    SELECT
      ...
      -- FIXED: Use actual breakeven from rankings cache, not recalculated
  ...
  SELECT
    ...
    -- vs BE = projection - breakeven (already correct, but uses correct breakeven now)
...
-- Rebuild snapshot with corrected breakeven values
