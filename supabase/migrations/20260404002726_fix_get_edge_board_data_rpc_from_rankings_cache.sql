/*
  # Fix get_edge_board_data RPC — rebuild from afl.player_rankings_cache

  ## Summary
  Drops all existing overloads of get_edge_board_data and rebuilds a single
  no-parameter version that reads exclusively from afl.player_rankings_cache.

  ## Changes
  - Drops all existing get_edge_board_data function variants
  - Creates new function with NO parameters (fixes 400 error from old limit_n param)
  - Assigns captain/breakout/trap sections from cache fields
  - Returns all fields the frontend expects
  - Grants execute to anon and authenticated

  ## Logic
  - captain: top captain_score players (active, not injured, not bye)
  - breakout: top value_score players below median price
  - trap: high risk_rating players (overpriced risk signals)
  - Sections deduplicated so a player only appears once
*/
...
GRANT EXECUTE ON FUNCTION public.get_edge_board_data() TO anon;
GRANT EXECUTE ON FUNCTION public.get_edge_board_data() TO authenticated;
