/*
  # Unification Phase 2: Cross-surface consistency validation

  ## Problem
  Market Watch snapshot and Edge Board were being built by separate cron jobs
  at different times from the pipeline, causing staleness vs rankings cache.

  ## Changes
  1. Add fn_validate_snapshot_consistency() — compares market watch + edge board
     values against rankings cache and logs any mismatches as warnings
  2. Add fn_check_confidence_distribution() — validates confidence_label spread
     is within healthy expected range (HIGH 10-25%, MEDIUM 40-60%, LOW 20-40%)
  3. Wire both checks into run_neeko_pipeline() as post-snapshot validation steps

  ## Notes
  - Validation functions are non-blocking (warnings only, pipeline continues)
  - Staleness is eliminated by the fact that snapshots are already built INSIDE
    the pipeline (steps 14+16) after cache populate (step 8)
  - The separate cron schedules that previously triggered snapshots independently
    are superseded by the pipeline-internal calls
*/

-- ─── 1. Cross-surface consistency validation function ─────────────────────────
...
-- ─── 2. Rebuild run_neeko_pipeline with consistency validation step ───────────
...
25, 0, 'Starting', v_run_start, v_run_start
...
-- STEP 8: Enrichment pass on rankings cache (writes confidence_label + all canonical fields)
v_step_start := clock_timestamp();
BEGIN
INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- ============================================================
-- NEW STEP 17: Consistency validation (post-snapshot check)
-- ============================================================
v_step_start := clock_timestamp();
BEGIN
INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- STEP 21: Auto-mark players for AI regen
v_step_start := clock_timestamp();
BEGIN
INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
