/*
  # Fix Edge Board Signal Vocabulary Mismatch

  ## Problem
  The `populate_mv_edge_board` function and `get_edge_board_data` RPC filter for
  legacy signal values ('STRONG_BUY', 'BUY', 'SELL', 'STRONG_SELL') but the
  `fn_populate_player_rankings_cache` and `populate_rankings_cache` functions write
  the current signal vocabulary ('STRONG_START', 'START', 'HOLD', 'SIT', 'STRONG_SIT').
  This causes the edge board to always be empty, resulting in 2-day-stale data.

  ## Changes
  1. Rebuild `populate_mv_edge_board` to use current signal vocabulary
  2. Rebuild `get_edge_board_data` RPC to use current signal vocabulary

  ## Signal Mapping
  - Captain/Must-Have: STRONG_START or START (was STRONG_BUY, BUY)
  - Breakout: STRONG_START (was STRONG_BUY)
  - Trap/Do-Not-Start: SIT or STRONG_SIT (was SELL, STRONG_SELL)
*/

-- Step 1: Fix populate_mv_edge_board to use current signal vocabulary
...
VALUES (
  ...
  'Edge board rebuilt (signal-aligned v3 START/SIT vocab): ' || v_inserted || ' rows upserted',
