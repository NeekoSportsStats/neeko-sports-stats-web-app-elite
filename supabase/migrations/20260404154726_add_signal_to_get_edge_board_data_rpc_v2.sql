/*
  # Add signal column to get_edge_board_data RPC (drop + recreate)

  ## Summary
  Drops and recreates get_edge_board_data to add signal and signal_tag to its
  output. The frontend Edge Board will use signalFromField(row.signal) instead
  of the legacy mapRecommendationToSignal(signal_tag, ai_recommendation) pattern.

  ## Changes
  - DROP + CREATE get_edge_board_data with two new output columns: signal, signal_tag
  - All existing output columns preserved
*/
