...
-- Fix admin_get_raw_without_cache_audit() to correctly classify players with
-- all-zero stat records as ZERO_STAT_DATA rather than NO_FORM_DATA_YET.
-- Previously these showed should_be_active=true which was a false positive —
-- the pipeline is correct to skip players with no actual stat contributions.
...
BEGIN
  ...
  SELECT
    ...
    -- Exclusion bucket — ZERO_STAT_DATA checked before NO_FORM_DATA_YET
    CASE
    ...
    CASE
      ...
      WHEN ra.total_stat_units = 0
        THEN 'All ' || ra.games_count || ' raw game row(s) have every stat column at zero. These are corrupted or pre-game roster records ingested by the provider feed, not real match performances. Pipeline correctly skips them.'
    ...
    -- should_be_active — false for zero-stat data (pipeline is correct to skip)
    CASE
