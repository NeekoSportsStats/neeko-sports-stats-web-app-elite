/*
  # Update get_stat_board_players and get_stat_board_team_rows Free Threshold to 4

  ## Summary
  Updates `is_free_match` threshold from `<= 2` to `<= 4` in two RPCs:
  - `get_stat_board_players` — updates the `active_fixture` CTE
  - `get_stat_board_team_rows` — updates the final SELECT

  ## Notes
  - Only the threshold condition changes; all other logic is preserved exactly.
  - Return types are unchanged.
*/

-- ── get_stat_board_players: update active_fixture CTE threshold ───────────────
-- We patch ONLY the is_free_match line inside active_fixture CTE.
-- The full function body is reproduced to safely apply the patch.
...
active_fixture AS (
  SELECT
    ...
    rf.match_label,
    rf.match_order,
    (rf.match_order <= 4) AS is_free_match
