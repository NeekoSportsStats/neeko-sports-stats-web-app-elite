/*
  # Fix run_neeko_pipeline() — Move Stale Marking Before AI Trigger

  ## Problem
  In the previous pipeline, the stale-marking step ran AFTER the AI generation trigger:
    - Step 9: trigger_generate_player_ai (fires edge function)
    - Step 21: mark_players_needing_regen (marks stale — TOO LATE)

  This meant that stat changes from the current pipeline run were NOT picked up
  until the NEXT pipeline run. Players could have fresh stats but stale AI for
  an entire day.

  ## Fix
  Add Step 8c: mark_player_ai_stale_after_stat_change
    - Runs AFTER Step 8 (rankings cache refresh — so current_input_hash is fresh)
    - Runs BEFORE Step 9 (AI trigger — so newly marked players are visible to worker)
    - Calls run_neeko_ai_enqueue() which now correctly detects stale regardless of NULL hashes
    - Logs marked count to system_logs

  Step 21 (mark_players_needing_regen) is kept but now runs as a validation pass
  after AI fires, confirming the stale queue is healthy rather than being the
  primary marking mechanism.

  ## Result
  Pipeline order for AI freshness:
    Step 7b: fn_populate_player_rankings_cache (seeds cache)
    Step 8:  populate_rankings_cache_from_source (enriches cache, updates cached_at)
    Step 8b: validate_rankings_completeness
    Step 8c: mark_player_ai_stale_after_stat_change (NEW — marks stale, enqueues regen)
    Step 9:  trigger_generate_player_ai (fires worker — sees freshly marked rows)
    ...
    Step 21: mark_players_needing_regen (secondary pass / validation)

  ## Security: no RLS changes, SECURITY DEFINER preserved, search_path unchanged
*/
...
INSERT INTO public.pipeline_runs (
  ...
  26, 0, 'Starting', v_run_start, v_run_start
...
-- STEP 8: Enrichment pass on rankings cache (writes confidence_label + all canonical fields, updates cached_at)
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- ============================================================
-- STEP 8c: Mark player AI stale after stat change (THE FIX)
-- Runs AFTER rankings cache is fresh, BEFORE AI generation trigger.
-- Uses fixed run_neeko_ai_enqueue() which no longer has the NULL hash guard bug.
-- ============================================================
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- STEP 9: Trigger AI generation (now sees freshly marked stale rows from step 8c)
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- ============================================================
-- CALIBRATION BLOCK (runs before market watch)
-- ============================================================
...
-- ============================================================
-- DOWNSTREAM SNAPSHOTS (run AFTER cache + calibration complete)
-- ============================================================

-- STEP 14: Market watch snapshot — reads from afl.player_rankings_cache
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- STEP 16: Edge board — reads from afl.player_rankings_cache
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- STEP 17: Consistency validation (post-snapshot check)
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- STEP 21: Secondary AI regen pass — validation check after AI fires.
--          Now operates as a health/catch-up pass rather than primary marking mechanism.
v_step_start := clock_timestamp();
BEGIN
  INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
