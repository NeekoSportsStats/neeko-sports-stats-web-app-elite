/*
  # Fix get_stat_board_team_rows: correct per-lens thresholds

  Drops and recreates the RPC with corrected threshold values:
  - Score:          70, 80, 90, 100, 110 pts       (unchanged)
  - Goals:          6, 8, 10, 12, 14 goals          (was 8/10/12/14/16)
  - Scoring Shots:  18, 22, 26, 30, 34 shots        (was 18/20/22/24/26)
  - Disposals:      300, 330, 360, 390, 420 disp    (was 300/325/350/375/400)
*/

DROP FUNCTION IF EXISTS public.get_stat_board_team_rows(integer, integer, integer, text, integer, integer);

CREATE FUNCTION public.get_stat_board_team_rows(
  p_season   integer,
...
BEGIN
  ...
  SELECT
    ...
    -- Hit rates JSONB with correct per-lens thresholds
    CASE p_lens
...
GRANT EXECUTE ON FUNCTION public.get_stat_board_team_rows TO anon, authenticated, service_role;
