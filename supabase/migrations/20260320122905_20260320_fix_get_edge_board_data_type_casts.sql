/*
  # Fix get_edge_board_data type casts for mv_edge_board branch

  value_score in afl.player_rankings_cache is double precision but the function
  returns numeric. Add explicit ::numeric casts where needed.
*/
...
BEGIN
  ...
  IF v_mv_count > 0 THEN
    ...
    SELECT
      ...
      COALESCE(c.price::numeric, m.price)                        AS price,
      COALESCE(c.price_change, m.price_change)                   AS price_change,
      COALESCE(c.price_change_pct::numeric, m.price_change_pct)  AS price_change_pct,
      COALESCE(c.value_score::numeric, m.value_score)            AS value_score,
  ...
  ELSE
    ...
    sectioned AS (
      SELECT player_id, player_name, team, "position",
        ...
        value_score::numeric, value_tag, ai_summary, recommendation_color,
