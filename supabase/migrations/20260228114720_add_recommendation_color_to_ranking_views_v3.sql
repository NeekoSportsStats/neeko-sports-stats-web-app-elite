/*
  # Add recommendation_color to all ranking views and RPC (v3)

  ## Changes
  - Rebuilds v_rankings_premium with recommendation_color added after ai_analysis
  - Rebuilds v_rankings_master to expose recommendation_color
  - Rebuilds get_rankings_free RPC to return recommendation_color

  ## Purpose
  Exposes recommendation_color to the frontend so the badge component
  can render the ELITE CAPTAIN gold glow using the stored hex value.
*/
...
SELECT
  ...
  proj.team,
  CASE
    WHEN pc."position" ~~* '%defender%' THEN 'DEF'
    WHEN pc."position" ~~* '%forward%'  THEN 'FWD'
    WHEN pc."position" ~~* '%mid%'      THEN 'MID'
    WHEN pc."position" ~~* '%ruck%'     THEN 'RUC'
    ELSE 'MID'
  END AS "position",
