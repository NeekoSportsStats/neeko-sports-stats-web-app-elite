/*
  # Fix Rankings Views - Use Cache Breakeven Directly

  ## Summary
  Remove COALESCE from breakeven column since all values are now populated
  in the cache (either actual 2026 avg or 60 fallback for rookies).

  ## Changes
  All breakeven values are now in cache, so views can use c.breakeven directly
  without COALESCE fallback.

  ## Impact
  - v_rankings_master: c.breakeven (instead of COALESCE(c.breakeven, 60))
  - v_rankings_free: inherits from master
*/

-- ============================================================
-- 1. Rebuild v_rankings_master - Use cache breakeven directly
-- ============================================================
...
SELECT
  ...
  -- Pricing (FIXED: Use cache value directly, no COALESCE needed)
  c.price,
  c.prev_price,
  c.price_change,
  c.price_change_pct,
...
'Rankings master view - Uses cache breakeven directly (2026 season avg or 60 for rookies)';
...
SELECT
  ...
  -- Pricing (breakeven from master, already correct)
  p.price,
  p.prev_price,
  p.price_change,
  p.price_change_pct,
...
'Rankings free tier view - Top 100 players with 2026 season avg breakeven from cache';
