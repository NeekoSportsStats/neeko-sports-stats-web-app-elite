/*
  # Rankings Completeness Validation

  ## Summary
  Adds a validation function that runs after the rankings cache is populated to
  ensure every active player has non-null projection, breakeven, and value_score.
  If any nulls are found, they are repaired in-place and the issue is logged to
  system_logs. The pipeline step is marked as an error if data was truly missing.

  ## New Functions
  - `afl.fn_validate_rankings_completeness()` — checks null counts, repairs them,
    logs to system_logs, raises a WARNING so the pipeline step is marked as error
    if data was missing.

  ## Changes to run_neeko_pipeline
  - Inserts a new step "validate_rankings_completeness" between cache refresh
    (step 8) and AI generation trigger (step 9).
  - total_tasks bumped from 22 to 23.

  ## Repair logic
  - breakeven:    COALESCE(season_avg, projection_final, 0)
  - value_score:  projection_final - repaired_breakeven
  - projection:   COALESCE(season_avg, last_3_avg, last_5_avg, 60)

  ## Observability
  - Clean run: no log entry, step status = success
  - Dirty run: system_logs entry at level=warning, step status = error
*/

-- ─── VALIDATION + SELF-HEAL FUNCTION ────────────────────────────────────────
...
BEGIN
  ...
  IF v_null_breakeven > 0 THEN
    ...
    SET breakeven_canonical = GREATEST(COALESCE(season_avg, projection_final, 0), 0),
        breakeven           = GREATEST(COALESCE(season_avg, projection_final, 0), 0)
...
GRANT EXECUTE ON FUNCTION afl.fn_validate_rankings_completeness() TO service_role;
...
BEGIN
  ...
  BEGIN
    ...
    INSERT INTO public.pipeline_runs (
      ...
      23, 0, 'Starting', v_run_start, v_run_start
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      INSERT INTO public.system_logs (event_type, source, log_level, message, metadata) VALUES
        ('pipeline_step_error', 'cron:neeko_full_pipeline', 'error',
         'validate_rankings_completeness failed: ' || SQLERRM,
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      VALUES ('pipeline_step_error', 'cron:neeko_full_pipeline', 'error',
        'refresh_fantasy_market_matches failed: ' || SQLERRM, jsonb_build_object('run_id', v_run_id));
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      INSERT INTO public.system_logs (event_type, source, log_level, message, metadata) VALUES
        ('pipeline_step_error', 'cron:neeko_full_pipeline', 'error',
         'refresh_accuracy_metrics failed: ' || SQLERRM, jsonb_build_object('run_id', v_run_id));
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      INSERT INTO public.system_logs (event_type, source, log_level, message, metadata) VALUES
        ('pipeline_step_error', 'cron:neeko_full_pipeline', 'error',
         'mark_players_needing_regen failed: ' || SQLERRM, jsonb_build_object('run_id', v_run_id));
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      INSERT INTO public.system_logs (event_type, source, log_level, message, metadata) VALUES
        ('pipeline_step_error', 'cron:neeko_full_pipeline', 'error',
         'ai_health_guard failed: ' || SQLERRM, jsonb_build_object('run_id', v_run_id));
    END;
    ...
    EXCEPTION WHEN OTHERS THEN
      ...
      INSERT INTO public.system_logs (event_type, source, log_level, message, metadata) VALUES
        ('pipeline_step_error', 'cron:neeko_full_pipeline', 'error',
         'pipeline_completion_check failed: ' || SQLERRM, jsonb_build_object('run_id', v_run_id));
    END;
