/*
  # Fix 10: Recalibrate Market Watch Snapshot Categorisation Thresholds

  ## Summary
  The market.build_market_watch_snapshot() function was written expecting raw dollar-scale
  value_score values (e.g. > 50,000). However afl.player_rankings_cache stores normalized
  value scores in the range 15–423 (p25=86, p50=96, p75=109, p90=125).

  This caused ALL 628 players to fall into the 'monitor' / 'fade' catch-all categories
  because none had value_score > 30,000.

  ## Fix
  Rebuilds market.build_market_watch_snapshot() with recalibrated thresholds:
  - buy:          value_score > 150  (top ~15% — genuinely good value)
  - cash_cow:     value_score > 120 AND price < 400000 (value + cheap)
  - sell_now:     value_score < 50   (bottom ~5% — overpriced)
  - sell_consider: value_score BETWEEN 50 AND 75 (below median)
  - fade:         price >= 500000 AND risk_rating >= 70
  - monitor:      everything else

  Also recalibrates:
  - trade_score formula (was adding raw value_score; now uses proportional neeko_rating)
  - breakout_flag (was checking ceiling_val >= 130; now >= 100 given normalized scale)
  - momentum thresholds (was using 60000/30000; now 50/30 matching value_score scale)
  - peak_price / est_price formula: last3_avg * 7200 is correct for AFL Fantasy pricing
*/
...
BEGIN
  ...
  categorised AS (
    SELECT *,
      CASE peak_r
        ...
        ELSE 'sell'
      END AS peak_st,
      -- RECALIBRATED: value_score is 15-423 scale (p50=96, p75=109, p90=125)
      CASE
        ...
        ELSE 'monitor'
      END AS cat,
      -- RECALIBRATED: momentum thresholds (was 60000/30000; now 50/30 on 0-500 scale)
      CASE
        ...
        ELSE 'stable'
      END AS mom_label,
      -- RECALIBRATED: breakout threshold (ceiling now 0-200 range)
      CASE
  ...
  SELECT
    ...
    CASE
      ...
      ELSE 'HOLD'
    END                 AS action,
    -- RECALIBRATED: trade_score uses neeko_rating (0-200 scale) not raw value_score
    ROUND(
