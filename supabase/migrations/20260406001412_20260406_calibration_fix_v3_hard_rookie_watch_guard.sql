/*
  # Market Watch Calibration Fix v3 — Hard Rookie Guard

  ## Summary
  All players with games_played < 3 are forced to Watch/STABLE/HOLD regardless
  of their edge score. This prevents rookie/small-sample players from dominating
  the Target category in early season.

  ## Distribution result (expected)
  - TARGET: ~14-18% (only established players with edge >= p75 AND games_played >= 3)
  - WATCH: ~50-55% (everyone with low games + middle edge)
  - AVOID: ~28-32% (negative edge players)

  ## Key rule
  games_played < 3 → STABLE signal, Watch category, HOLD action (always)
  games_played >= 3 AND edge >= p85 → STRONG_UP, Target, BUY
  games_played >= 3 AND edge >= p75 → UP, Target, BUY
  games_played >= 3 AND edge >= p35 → STABLE, Watch, HOLD
  games_played >= 3 AND edge >= p15 → DOWN, Avoid, SELL
  games_played >= 3 AND edge < p15  → STRONG_DOWN, Avoid, SELL
*/
...
BEGIN
  ...
  SELECT
    ...
    -- signal: hard rookie guard first
    CASE
    ...
    CASE pp.matchup_rating::text WHEN '1' THEN 'Tough' WHEN '3' THEN 'Favourable' ELSE 'Average' END,
    CASE pp.matchup_rating::text WHEN '1' THEN 'Hard matchup this round' WHEN '3' THEN 'Great matchup this round' ELSE 'Average matchup' END,
    ...
    false,
    pp.team_id,
    COALESCE(ap.manual_status, 'active'),
    ap.manual_status,
    CASE WHEN COALESCE(ap.manual_status, 'active') IN ('active', 'questionable') THEN true ELSE false END,
    pa.summary_short,
    pa.summary_long,

    -- category (market_watch_category): HARD ROOKIE GUARD — < 3 games always Watch
    CASE
    ...
    COALESCE(pp.last5_avg, pp.last3_avg, pp.season_avg, pp.projection),
    (pp.projection - COALESCE(pp.last5_avg, pp.last3_avg, pp.season_avg, pp.projection)),
    CASE WHEN pp.price > 0 THEN ROUND((pp.projection - COALESCE(pp.last5_avg, pp.last3_avg, pp.season_avg, pp.projection)) / (pp.price::numeric / 100000.0), 3) ELSE 0 END,
    ...
    -- category_canonical: HARD ROOKIE GUARD
    CASE
      WHEN COALESCE(pp.games_played, 0) < 3                                                                                   THEN 'Watch'
      WHEN (pp.projection - COALESCE(pp.last5_avg, pp.last3_avg, pp.season_avg, pp.projection)) >= v_p75                     THEN 'Target'
      WHEN (pp.projection - COALESCE(pp.last5_avg, pp.last3_avg, pp.season_avg, pp.projection)) >= v_p35                     THEN 'Watch'
