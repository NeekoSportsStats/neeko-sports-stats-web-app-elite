...
/*
  # Rebuild market.build_market_watch_snapshot

  ## Problem
  The existing function depends on two missing objects:
  - afl.v_player_round_projections_2026  (does not exist)
  - public.v_rankings_with_value         (does not exist)

  This caused the snapshot to only populate 22 players from a stale run.

  ## Fix
  Rewrite to source all data from afl.player_rankings_cache (640 players)
  and afl.player_games for last-3 avg calculation.

  ## Key changes
  - All double precision values cast to numeric for ROUND() compatibility
  - cat_rank cap removed — includes all eligible players per category
  - rationale column added to best_trades INSERT (was missing)
  - Season/round detected from afl.player_games instead of missing projection view
*/
...
-- Run immediately to populate all 640 players
