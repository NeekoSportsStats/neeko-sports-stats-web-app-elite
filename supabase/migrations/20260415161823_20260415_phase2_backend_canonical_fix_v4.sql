...
/*
  # Phase 2 Backend Canonical Fix (v4)

  ## Summary
  Targeted fixes to the core pipeline cache-populate function and captain RPC.

  ## Key Fixes

  ### fn_populate_player_rankings_cache()
  1. confidence_label: Fixed thresholds on confidence_score_100 (HIGH >= 72, MEDIUM >= 52)
     instead of PERCENT_RANK() thresholds on percentile position
  2. captain_score: Now written — formula: projection*0.40 + ceiling*0.30 + confidence_score_100*0.20 + last_3_avg*0.10
  3. captain_rating: Derived from captain_score (HIGH >= 130, MEDIUM >= 115, LOW otherwise)
  4. decision_score weights: Removed duplicate value_z (was identical to edge_z, inflating edge weight to 0.50)
     New weights: edge_z*0.40 + trend_z*0.25 + form_z*0.20 + conf_z*0.15
  5. ceiling_estimate and floor_estimate removed from INSERT (they are generated columns auto-derived from ceiling/floor)

  ### get_captain_recommendations_free()
  - Rebuilt to query player_rankings_cache directly (avoids view dependency)
  - Uses integer player_id correctly
  - Returns results now that captain_score is populated
...
-- ============================================================
-- 1. Rebuild fn_populate_player_rankings_cache with all fixes
-- ============================================================
...
pop_stats AS (
SELECT
AVG(LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric))     AS edge_mean,
NULLIF(STDDEV_POP(LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric)), 0) AS edge_std,
...
THEN (LEAST(GREATEST(se.edge_raw, -40.0::numeric), 40.0::numeric) - ps.edge_mean) / ps.edge_std
...
SELECT *,
ROUND(
(edge_z * 0.40::numeric) + (trend_z * 0.25::numeric) + (form_z * 0.20::numeric) + (conf_z * 0.15::numeric)
...
SELECT *,
CASE
WHEN confidence_score_100_val >= 72.0::numeric THEN 'HIGH'
WHEN confidence_score_100_val >= 52.0::numeric THEN 'MEDIUM'
ELSE                                                'LOW'
...
+ (confidence_score_100_val * 0.20::numeric)
+ (LEAST(COALESCE(last_3_avg, last_5_avg, season_avg, 0::numeric), 200.0::numeric) * 0.10::numeric)
, 1) >= 130.0::numeric THEN 'HIGH'
WHEN ROUND(
(projection_final * 0.40::numeric)
+ (ceiling * 0.30::numeric)
+ (confidence_score_100_val * 0.20::numeric)
+ (LEAST(COALESCE(last_3_avg, last_5_avg, season_avg, 0::numeric), 200.0::numeric) * 0.10::numeric)
, 1) >= 115.0::numeric THEN 'MEDIUM'
ELSE 'LOW'
...
SELECT *,
ROUND(
100.0::numeric * PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric))::numeric
, 1) AS edge_pct_val,
CASE
WHEN PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric))::numeric >= 0.85::numeric THEN 'Elite Value'
WHEN PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric))::numeric >= 0.65::numeric THEN 'Strong Value'
WHEN PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric))::numeric >= 0.35::numeric THEN 'Fair Value'
WHEN PERCENT_RANK() OVER (ORDER BY LEAST(GREATEST(edge_raw, -40.0::numeric), 40.0::numeric))::numeric >= 0.15::numeric THEN 'Thin Value'
ELSE 'Poor Value'
...
VALUES (
'cache_seed',
'fn_populate_player_rankings_cache (phase2_v4_fixed_confidence_captain_score): ' || v_inserted || ' rows upserted',
NOW()
)
...
-- ============================================================
-- 2. Run cache populate immediately to backfill all fixes
-- ============================================================
SELECT afl.fn_populate_player_rankings_cache();

-- ============================================================
-- 3. Fix get_captain_recommendations_free
--    - query cache directly (not view) to avoid dependency issues
--    - player_id is integer, cast to text for return
--    - no more captain_score IS NOT NULL filter returning 0 rows
-- ============================================================
...
GRANT EXECUTE ON FUNCTION public.get_captain_recommendations_free(int) TO anon, authenticated;
