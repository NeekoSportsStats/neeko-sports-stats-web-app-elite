/*
  # FIX 2 — Restore Step 8 enrichment pass in run_neeko_pipeline()

  ## Problem
  Step 8 in run_neeko_pipeline() had its core function call commented out:
    --PERFORM afl.populate_rankings_cache_from_source()
  
  This meant Step 8 always logged success in 0ms without doing any work.
  afl.fn_populate_player_rankings_cache() (Step 7b) only INSERTs new rows with
  basic seeded values. afl.populate_rankings_cache() (called by populate_rankings_cache_from_source)
  is the enrichment pass that computes blended breakeven, signal, edge, category,
  and writes AI summary columns from ai.player_ai_analysis.

  Without Step 8, every pipeline run produced stale breakeven, edge and signal values
  because the enrichment UPDATE was never executed.

  ## Fix
  Uncomment the PERFORM call in Step 8 so afl.populate_rankings_cache_from_source()
  actually runs each pipeline cycle.

  ## Notes
  - afl.populate_rankings_cache_from_source() is a thin wrapper around afl.populate_rankings_cache()
  - afl.populate_rankings_cache() does an UPDATE (not INSERT) — it enriches rows already seeded by Step 7b
  - This function implements the correct blended breakeven formula (0gp=last5, 1-2gp=40/60, 3+gp=season_avg)
  - The total_tasks count (24) remains correct — Step 8 was always counted, just doing nothing
*/
...
-- Computes blended breakeven, signal, edge, category, and syncs AI summary columns.
v_step_start := clock_timestamp();
BEGIN
INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
