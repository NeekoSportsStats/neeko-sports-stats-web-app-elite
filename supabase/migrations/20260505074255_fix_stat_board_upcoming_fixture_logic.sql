/*
  # Fix Stat Board: Upcoming Fixture Logic

  ## Problem
  Both `get_stat_board_matches` and `get_stat_board_players` resolve the round
  using MAX(week) FROM afl.player_games, which returns the last *completed* week
  (currently week 8). This causes the match selector to show already-played games
  rather than the next upcoming fixtures.

  ## Fix

  ### resolved_round logic (new)
  Priority:
  1. Explicit p_round parameter — used as-is
  2. Next upcoming round — MIN(week) FROM afl.games WHERE game_date > NOW()
     (the earliest week that has at least one game in the future)
  3. Fallback — MAX(week) FROM afl.player_games (last round with completed stats)
     Used only when no future games exist (end of season).

  ### Historical stats boundary (get_stat_board_players)
  Changed from:   week <= resolved_round  (could include current fixture week)
  Changed to:     week < fixture_week     (strictly before the selected fixture)
  This guarantees we never include the selected upcoming game's stats even if
  partial data exists.

  ## Tables/functions modified
  - `get_stat_board_matches` — rebuild resolved_round CTE
  - `get_stat_board_players` — rebuild resolved_round CTE + season_games WHERE clause
*/

-- ── get_stat_board_matches ────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS get_stat_board_matches(integer, integer);

CREATE OR REPLACE FUNCTION get_stat_board_matches(
  p_season  integer,
...
-- Resolve which round/week to show in the selector.
-- Priority: explicit p_round → next upcoming round → last completed round (fallback)
resolved_round AS (
  SELECT COALESCE(
    p_round,
    -- Next upcoming round: earliest week that has at least one future game
    (
      SELECT MIN(week)
      FROM afl.games
    ...
    -- Fallback: last round with completed player stats (end-of-season)
    (
      SELECT MAX(week)
...
GRANT EXECUTE ON FUNCTION get_stat_board_matches(integer, integer) TO anon, authenticated;


-- ── get_stat_board_players ────────────────────────────────────────────────────

DROP FUNCTION IF EXISTS get_stat_board_players(integer, integer, integer, text, numeric, text, integer, text, integer, integer);

CREATE OR REPLACE FUNCTION get_stat_board_players(
  p_season          integer,
  p_round           integer  DEFAULT NULL,
...
-- ── 2. Resolve selected round (upcoming fixture logic) ────────────────────────
-- Priority: explicit p_round → next upcoming round → last completed round
resolved_round AS (
  SELECT COALESCE(
    p_round,
    -- Next upcoming round: earliest week with at least one future game
    (
      SELECT MIN(week)
      FROM afl.games
      WHERE season = p_season
    ...
    -- Fallback: last round with completed stats (end of season)
    (
      SELECT MAX(week)
      FROM afl.player_games
      WHERE season = p_season
...
-- ── 4. Active fixture (selected match or first of round) ──────────────────────
active_fixture AS (
  SELECT
    rf.game_id,
    rf.week,
...
-- ── 5. Each player's current team (from most recent completed game) ───────────
...
-- ── 7. All weeks each team had a scheduled game this season ───────────────────
...
-- ── 8. Historical completed games — strictly before the selected fixture week ──
-- Uses week < fixture_week to guarantee the upcoming game is never included,
-- even if partial data were to exist for it.
season_games AS (
  SELECT
    pg.player_id AS pid,
    pg.week      AS wk,
    ...
    END AS sv
  FROM afl.player_games pg
  -- Strictly prior to the selected fixture week
  CROSS JOIN active_fixture af
  WHERE pg.season = p_season
    AND pg.week < af.week
...
timeline_slots AS (
  ...
  CROSS JOIN LATERAL (
    -- Weeks team had a scheduled game (prior to fixture)
    SELECT week FROM team_played_weeks
    WHERE team_id = pct.team_id
      AND season  = p_season
      AND week < (SELECT week FROM active_fixture)
    UNION
    -- Bye weeks: weeks on the overall schedule where this team had no game
...
GRANT EXECUTE ON FUNCTION get_stat_board_players(integer, integer, integer, text, numeric, text, integer, text, integer, integer) TO anon, authenticated;
