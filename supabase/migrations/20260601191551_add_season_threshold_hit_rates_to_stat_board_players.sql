/*
  # Add season_threshold_hit_rates to get_stat_board_players RPC

  ## Summary
  Changes the `get_stat_board_players` function to expose a new `season_threshold_hit_rates`
  column alongside the existing `all_threshold_hit_rates` column.

  ## What changes
  - **`agg` CTE**: Adds season-level hit counts (sh15/sh20/sh25/sh30/shg1/shg2/shg3/shg4)
    using NO `rn <= 10` cap, so they count all season games for each player.
  - **RETURNS TABLE**: Adds `season_threshold_hit_rates jsonb` column (same shape as
    `all_threshold_hit_rates` but denominator is `total_g` not `cnt10`).
  - All existing columns and logic are preserved unchanged.
...
p_round,
-- Priority 3: canonical current round
...
-- Existing: last-10 based hit rates
CASE (SELECT lens FROM params)
WHEN 'disposals' THEN
jsonb_build_object(
...
-- New: full-season hit rates (denominator = total_g, no last-10 cap)
CASE (SELECT lens FROM params)
WHEN 'disposals' THEN
jsonb_build_object(
...
-- Grant access to match existing permissions
