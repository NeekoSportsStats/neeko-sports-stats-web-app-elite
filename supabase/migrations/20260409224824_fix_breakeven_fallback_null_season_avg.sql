/*
  # Fix breakeven fallback for players with NULL season_avg

  ## Root Cause
  121 players in afl.player_rankings_cache have season_avg = NULL because they
  have games_played = 0 in the current season (2026). These are players who have
  not yet played this season (pre-season, injured, returning from bye, etc.).

  The populate function used GREATEST(COALESCE(season_avg, 0), 0) which falls
  back to 0 when season_avg is NULL. This causes:
  - breakeven = 0 for 121 players
  - edge = projection (not edge)
  - value_score = projection (incorrect)
  - All 121 players get STRONG_START signal (massively wrong)

  ## Fix
  Use best available average for breakeven:
    COALESCE(season_avg, last5_avg, last3_avg, last10_avg, projection)

  Priority order:
  1. season_avg — current season average (most relevant)
  2. last5_avg  — recent form (5-game rolling avg, includes prior season)
  3. last3_avg  — short-term form
  4. last10_avg — longer rolling avg
  5. projection — last resort (edge = 0, HOLD signal)

  This ensures every player with any form data gets a meaningful breakeven.

  ## Changes
  - Rebuilds afl.fn_populate_player_rankings_cache() with fallback breakeven logic
  - Rebuilds afl.populate_rankings_cache() enrichment pass with same fallback
  - Runs cache repopulate to fix all 121 affected players immediately
*/

-- ============================================================
-- STEP 1: Rebuild seed function with fallback breakeven
-- ============================================================
...
SELECT
  ...
  -- BREAKEVEN: best available average (season_avg first, fallback to rolling avgs)
  -- This handles players with 0 games in 2026 (pre-season, injured, returning)
  ROUND(
    GREATEST(
      COALESCE(
...
SELECT
  *,
  ...
  -- ACTION VOCABULARY: STRONG_START / START / HOLD / SIT / STRONG_SIT
  CASE
...
SELECT
  ...
  games_played, is_available, manual_status, status,
  -- breakeven stored in all three slots
  breakeven_val, breakeven_val, breakeven_val,
  -- edge stored in both slots
  edge_raw, edge_raw,
  -- value_score = edge directly
  edge_raw, edge_raw,
  -- signal (all signal columns use same START/SIT vocab)
  sig, sig,
  CASE sig
  ...
  END,
  sig,
  -- category_canonical: Target / Watch / Avoid
  CASE sig
  ...
  END,
  -- action_canonical: START / HOLD / SIT
  CASE sig
  ...
  END,
  -- market_watch_category: Target / Watch / Avoid
  CASE sig
    WHEN 'STRONG_START' THEN 'Target'
    WHEN 'START'        THEN 'Target'
    WHEN 'HOLD'         THEN 'Watch'
...
VALUES (
  'cache_seed',
  'fn_populate_player_rankings_cache: ' || v_inserted || ' rows upserted (breakeven=COALESCE(season_avg,last5_avg,last3_avg,last10_avg,projection))',
  NOW()
)
...
-- ============================================================
-- STEP 2: Rebuild enrichment/update pass with same fallback
-- ============================================================
...
BEGIN
  ...
  SET
    ...
    -- BREAKEVEN: best available average
    ...
    -- VALUE SCORE = edge directly
    value_score           = pp.edge_val,
    value_score_canonical = pp.edge_val,

    -- SIGNAL vocab: STRONG_START / START / HOLD / SIT / STRONG_SIT
    signal                = pp.sig,
    signal_tag            = pp.sig,
  ...
  VALUES (
    'cache_enrichment',
    'populate_rankings_cache: ' || v_updated || ' rows updated (breakeven=COALESCE(season_avg,last5_avg,last3_avg,last10_avg,projection))',
    NOW()
  )
  ON CONFLICT DO NOTHING;
...
-- ============================================================
-- STEP 3: Run the seed function immediately to fix all 121 affected players
-- ============================================================
