/*
  # Phase 2.5 — Final System Hardening: Canonical Lock + Confidence Calibration + Metric Cleanup

  ## Summary
  This migration is the FINAL integrity pass for the AFL scoring system.
  It does NOT introduce new features. It locks the canonical system so it
  cannot drift, duplicate, or silently break again.

  ## Changes Made

  ### 1. Confidence Distribution Recalibration
  Current state: HIGH=9.3%, MEDIUM=71.2%, LOW=19.5%
  Target: HIGH ≈ 20-25%, MEDIUM ≈ 50-60%, LOW ≈ 20-25%

  Live data shows:
  - confidence_score_100 range: 36.7 – 81.7
  - p75 = 68.4, p80 = 69.0, p85 = 69.9

  New thresholds: HIGH >= 67 (~top 25%), MEDIUM >= 50 (middle 50%), LOW < 50

  ### 2. Canonical Field Lock
  - edge_canonical is computed ONCE as projection - breakeven
  - ALL aliases (edge, value_score, value, value_score_canonical) are set to edge_canonical
  - ALL signal aliases (signal, signal_tag, signal_canonical) are set to action_canonical
  - breakeven, baseline are set to breakeven_canonical
  - NO independent re-assignments anywhere

  ### 3. Trend Score Redefinition (Phase 4 — Metric Cleanup)
  trend_score has 0.971 correlation with edge_canonical — near-identical.
  Redefine trend_score to represent ONLY the short vs long term delta:
  trend_score = last_3_avg - season_avg (form momentum, not edge)
  This gives trend a UNIQUE signal not captured by edge.

  ### 4. Decision Score Weight Rebalance
  With trend now measuring form momentum (unique), the weights are updated:
  edge_z: 0.45  (primary driver — projection vs breakeven)
  trend_z: 0.20  (form momentum — distinct from edge now)
  form_z:  0.20  (raw form score)
  conf_z:  0.15  (reliability)

  ### 5. Pipeline Documentation
  All canonical assignments are clearly documented in SQL comments.
  No metric is assigned more than once.

  ### 6. Dead Column Cleanup
  - conf_pct_val (PERCENT_RANK result) still stored as confidence_percentile
    This is kept for diagnostics only. Not used for labels.

  ## Security
  No RLS changes — function is SECURITY DEFINER, existing policies unchanged.

  ## Important Notes
  1. After migration: SELECT afl.fn_populate_player_rankings_cache() is called
  2. All alias columns derive from canonical — no independent paths
  3. Thresholds are ABSOLUTE — never dynamic or percentile-based
*/

-- ─── Step 1: Rebuild fn_populate_player_rankings_cache with full canonical lock ───
...
VALUES (
  'cache_seed',
  'fn_populate_player_rankings_cache (phase25_hardened): ' || v_inserted || ' rows upserted',
  NOW()
)
...
-- ─── Step 2: Run the cache refresh immediately ─────────────────────────────────
SELECT afl.fn_populate_player_rankings_cache();

-- ─── Step 3: Verify distribution after calibration ────────────────────────────
DO $$
DECLARE
  v_high int; v_med int; v_low int; v_total int;
BEGIN
  SELECT
    ...
    COUNT(*)
  INTO v_high, v_med, v_low, v_total
  ...
  VALUES (
    'phase25_validation',
    format(
      'Confidence distribution: HIGH=%s (%s%%), MEDIUM=%s (%s%%), LOW=%s (%s%%) of %s total',
      v_high, ROUND(100.0*v_high/NULLIF(v_total,0),1),
      v_med,  ROUND(100.0*v_med /NULLIF(v_total,0),1),
      v_low,  ROUND(100.0*v_low /NULLIF(v_total,0),1),
      v_total
    ),
    NOW()
  ) ON CONFLICT DO NOTHING;
