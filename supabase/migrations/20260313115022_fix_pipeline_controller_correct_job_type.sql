/*
  # Fix Pipeline Controller — Correct job_type filter in drain loop

  The drain loop used job_type = 'ranking_reco' but the actual value in
  ai_generation_queue is 'ranking_recommendation'. This patch corrects both
  the pending_before check and the pending_now loop check so the drain loop
  actually triggers and exits correctly.
*/
...
--
-- job_type = 'ranking_recommendation' (verified against live queue data)
-- Each worker call processes ~30 jobs.
-- 30 calls × 30 jobs = up to 900 jobs cleared per pipeline run.
-- Loop exits immediately once queue is empty — no wasted calls.
-- 1 second sleep between calls to prevent edge function thundering herd.
--------------------------------------------------------------------------
SELECT COUNT(*)
INTO   pending_before
