/*
  # Fix fn_populate_player_rankings_cache - PERCENT_RANK double precision cast

  ## Problem
  PERCENT_RANK() window function always returns `double precision` in PostgreSQL.
  Any ROUND(double_precision, integer) call fails because PostgreSQL only has
  ROUND(numeric, integer). The fix: cast PERCENT_RANK()::numeric before arithmetic.

  ## Fix
  - src_with_conf_pct: cast PERCENT_RANK()::numeric
  - src_with_value_band: cast all PERCENT_RANK()::numeric
...
SELECT *,
  ROUND(
    (edge_z * 0.40::numeric) + (trend_z * 0.20::numeric) + (form_z * 0.15::numeric) + (conf_z * 0.15::numeric) + (value_z * 0.10::numeric)
...
SELECT *,
  ...
  -- FIX: PERCENT_RANK() returns double precision — must cast to numeric before ROUND
  ROUND(
...
SELECT *,
  CASE
    WHEN conf_pct_val >= 82.0::numeric THEN 'HIGH'
    WHEN conf_pct_val >= 30.0::numeric THEN 'MEDIUM'
    ELSE                                    'LOW'
...
SELECT *,
  ...
  CASE
    WHEN conf_pct_val >= 82.0::numeric THEN
      CASE
      ...
      END
    WHEN conf_pct_val >= 30.0::numeric THEN
      CASE
