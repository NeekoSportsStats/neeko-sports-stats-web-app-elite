/*
  # Fix refresh_player_rankings_cache — correct source views v4

  ## Summary
  Uses correct source views:
  - `afl.v_player_rankings_full` — pre-computed base with all ranking columns
  - `afl.v_ai_player_metrics` — raw start_confidence and bust_risk for NTILE normalisation
  - `afl.players` — active player filter

  All column names match actual schema. Correct join on player_id (integer in cache,
  but note player_id type must be consistent).

  ## Recommendation thresholds (v2 calibration)
  Anchored to actual data distribution (n=640 active players, 2026 season):
