...
/*
  # Calibration Phase 15: Pipeline Automation + Weekly Cron Job

  ## Summary
  1. Extends public.run_neeko_pipeline() to include calibration steps:
     - Step 7: snapshot next-round projections
     - Step 8: refresh projection errors
     - Step 9: refresh calibration tables
     - Step 10: rebuild calibrated confidence
     - Step 11: refresh bias adjustments
  
  2. Adds a weekly pg_cron job:
     Tuesday 2:30 AM Melbourne time (AEDT = UTC+11, AEST = UTC+10)
     Using UTC+10 safe window (16:30 UTC Monday) to handle DST safely.

  ## Notes
  - The pipeline total_tasks count is updated from 6 to 11
  - All new steps are wrapped in individual BEGIN/EXCEPTION blocks
    so a failure in one step never blocks the rest of the pipeline
  - Existing 6 steps are preserved exactly
*/
...
BEGIN
  ...
  INSERT INTO public.pipeline_runs (
    ...
    11, 0, 'Starting', v_run_start, v_run_start
  ...
  -- ── STEP 4: rebuild_player_projection ────────────────────────────────
  v_step_start := clock_timestamp();
  BEGIN
    INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
  ...
  -- ── STEP 6: trigger generate-player-ai (fire-and-forget) ─────────────
  v_step_start := clock_timestamp();
  BEGIN
    INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
  ...
  -- ── STEP 7: snapshot projections for next round ───────────────────────
  v_step_start := clock_timestamp();
  BEGIN
    INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
...
-- -----------------------------------------------------------------------
-- Weekly cron: Tuesday 2:30 AM Melbourne time
-- AEST = UTC+10, so 2:30 AM AEST = 16:30 UTC Monday
-- Safe for both AEST and AEDT (AEDT=UTC+11 → 15:30 UTC Mon = 2:30 AM Tue)
-- Using Monday 16:30 UTC covers both timezone states conservatively
-- -----------------------------------------------------------------------
SELECT cron.schedule(
  'weekly_model_improvement_tuesday_melb',
  '30 16 * * 1',   -- 16:30 UTC Monday = 2:30 AM Tuesday AEST
  $$SELECT public.run_weekly_model_improvement();$$
);
