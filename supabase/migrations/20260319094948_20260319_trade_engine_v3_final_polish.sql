/*
  # Trade Engine V3 — Final Polish

  ## Summary
  Polishes the trade recommendation engine with five targeted improvements:

  1. Raise minimum trade quality — filter out projection_gain < 5
  2. Reduce cash dominance — divide cash_delta by 3000 (was 2000)
  3. Add balanced trade bonus — +20 pts when cash_delta is between -50k and +150k
  4. Tighten sell repetition cap — max 3 trades per sell player (was 5)
  5. Clearer trade type classification — BALANCED now surfaces properly

  ## Trade Score Formula (V3)
    trade_score =
      (projection_gain * 4)
      + (cash_delta / 3000)
      + (buy.value_score * 2)
      + (sell.value_score * -1)
      + balanced_trade_bonus

    balanced_trade_bonus = 20 when cash_delta BETWEEN -50000 AND 150000, else 0

  ## Trade Type Logic (V3)
    - AGGRESSIVE_UPGRADE : projection_gain >= 12
    - CASH_GENERATION    : cash_delta > 200000
    - BALANCED           : everything else

  ## Changes vs V2d
  - projection_gain filter raised from > 3 to >= 5
  - cash_delta weight reduced from /2000 to /3000
  - balanced_trade_bonus of +20 added
  - sell_rn cap lowered from 5 to 3
  - trade_type thresholds updated (AGGRESSIVE now >= 12 pts, not > 10)
  - v_trade_best projection_gain filter already > 5 (unchanged)
*/
...
trade_pairs AS (
  SELECT
    ...
    CASE
      WHEN (b.buy_projection - s.sell_projection) >= 12 THEN 'AGGRESSIVE_UPGRADE'
