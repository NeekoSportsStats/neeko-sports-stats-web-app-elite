/*
  # Tighten SELL and tune BUY thresholds v5

  ## Summary
  Recalibrates SELL so it only targets genuinely overpriced players
  (expensive price + poor value + low projection), not cheap mid-tier players.
  Also slightly relaxes BUY to produce a more useful signal (~2-4% of pool).

  ## Changes
  - SELL: now requires price > 700,000 in addition to value ≤ 8.5 and projection < 55
    (prevents flagging cheap/mid-priced players who simply have moderate scores)
  - BUY: relaxed to value ≥ 11.0 (was 11.5) — slightly more inclusive
  ...
  - HOLD:  ~52%
  - SIT:   ~30%
  - SELL:  ~8% (overpriced premium players only)
*/
    ...
    SELECT
      ...
      CASE
        ...
        WHEN b.value_score >= 10.5
          ...
          THEN 'START'
        -- SELL: overpriced premium players only (expensive + poor value + low projection)
