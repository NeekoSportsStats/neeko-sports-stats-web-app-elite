/*
  # Restore ranking AI generation pipeline views

  ## Summary
  Two views required by the generate-player-ranking-recos edge function were missing:
  - v_ai_player_ranking_openai_inputs_2026: builds the openai_input_json payload per player
  - v_ai_rankings_generation_queue: staleness-based queue (50 rows, 3-day TTL)

  v_rankings_premium and v_rankings_master are also rebuilt to ensure they include
  recommendation_color (added in a recent patch) and remain consistent with the
  rest of the pipeline.

  ## Views Created/Replaced
  1. v_rankings_premium — source of truth for all ranking metrics
  2. v_ai_player_ranking_openai_inputs_2026 — input view with fixture context
  3. v_ai_rankings_generation_queue — staleness queue consumed by edge function
  4. v_rankings_master — unified frontend view including recommendation_color
...
-- ─── 2. v_ai_player_ranking_openai_inputs_2026 ───────────────────────────────
...
-- ─── 3. v_ai_rankings_generation_queue ───────────────────────────────────────
