/*
  # Phase 4 — Fix Pipeline Execution Order

  ## Problem
  Calibration steps (12-15) were running AFTER the market watch snapshot (step 10).
  This caused market watch to be built using previous run's calibration data — a 24-hour lag.

  ## Fix
  Move steps 12-15 (projection errors, model calibration, calibrated confidence, bias adjustments)
  to run BEFORE step 10 (build_market_watch_snapshot) and step 11 (snapshot_projections).

  ## New Canonical Pipeline Order
  ...
  7b. seed_rankings_cache (fn_populate_player_rankings_cache)
  8. refresh_rankings_cache (populate_rankings_cache_from_source)
  8b. validate_rankings_completeness
  9. trigger_generate_player_ai
  --- CALIBRATION NOW BEFORE MARKET WATCH ---
  10. refresh_projection_errors       ← moved up from step 12
  11. refresh_model_calibration        ← moved up from step 13
  12. refresh_calibrated_confidence    ← moved up from step 14
  13. refresh_bias_adjustments         ← moved up from step 15
  --- MARKET WATCH NOW USES CALIBRATED DATA ---
  14. build_market_watch_snapshot      ← was step 10
  15. snapshot_projections             ← was step 11
  16. refresh_edge_board
  17. refresh_fantasy_market_matches
  18. log_model_accuracy_summary
  19. refresh_accuracy_metrics
  20. mark_players_needing_regen
  21. ai_health_guard
  22. pipeline_completion_check

  No business logic changed. Only execution order corrected.
*/
...
BEGIN
  ...
  BEGIN
    ...
    -- ============================================================
    -- CALIBRATION BLOCK (moved before market watch — Phase 4 fix)
    -- ============================================================

    -- STEP 10 (was 12): Projection errors — base data for calibration
    v_step_start := clock_timestamp();
    BEGIN
      INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
    ...
    -- STEP 11 (was 13): Model calibration
    v_step_start := clock_timestamp();
    BEGIN
      INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
    ...
    -- STEP 12 (was 14): Calibrated confidence
    v_step_start := clock_timestamp();
    BEGIN
      INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
    ...
    -- STEP 13 (was 15): Bias adjustments
    v_step_start := clock_timestamp();
    BEGIN
      INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
    ...
    -- ============================================================
    -- MARKET WATCH (now runs with calibrated data)
    -- ============================================================

    -- STEP 14 (was 10): Market watch snapshot — NOW uses calibrated confidence + bias
    v_step_start := clock_timestamp();
    BEGIN
      INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
    ...
    -- STEP 15 (was 11): Snapshot projections
    v_step_start := clock_timestamp();
    BEGIN
      INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
    ...
    -- STEP 17: Fantasy market matches
    v_step_start := clock_timestamp();
    BEGIN
      INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
    ...
    -- STEP 18: Model accuracy summary
    v_step_start := clock_timestamp();
    BEGIN
      INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
    ...
    -- STEP 20: Auto-mark players for AI regen
    v_step_start := clock_timestamp();
    BEGIN
      INSERT INTO public.pipeline_steps (id, run_id, step_name, step_label, status, started_at)
