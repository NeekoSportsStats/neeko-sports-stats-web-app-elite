...
- `public.projection_accuracy` stores per-player, per-game projection vs actual
  (columns: `player_id`, `season`, `round_number`, `round_label`,
  `projected_score`, `actual_score`, `error`, `abs_error`, `within_10`,
  `game_id`, `created_at`).
...
- The existing `get_projection_review_summary()` and `get_projection_review()`
  RPCs are admin-gated inside the function body (`RAISE EXCEPTION 'Admin
  access required'`), so anon clients cannot call them. This new RPC fills
  that gap for content surfaces.

## New function
- `public.get_accuracy_round_summary(p_season integer DEFAULT 2026,
  p_round integer DEFAULT NULL)`
  - `p_season` defaults to 2026 (current season).
  - `p_round` NULL -> latest round with accuracy rows for the season.
  - `p_round` provided -> that specific round.
  - Returns one summary row with: round metadata, games count, avg + within-10
    ...
    individual calls (by abs_error) with player name, projected, and actual.
...
## Notes
1. `v_accuracy_base` is anon-SELECT-able, but `projection_accuracy` is the
   raw source of truth with fewer derived columns and is also anon-SELECT-able.
   We query `projection_accuracy` directly to avoid view-dependency drift.
2. Join path for `player_name`: `player_rankings_cache.player_name` on
   `player_id` (both `integer`). Confirmed column exists and is `text`.
3. Projection column is `projected_score` (numeric); actual is `actual_score`
   (numeric). The legacy `projection` column is also populated but
   `projected_score` is the canonical field written by
   `refresh_projection_accuracy()`.
4. Best/worst call selection uses `abs_error` descending/ascending, filtered
   to rows where both `projected_score` and `actual_score` are non-null and
   `actual_score > 0` (excludes DNP / injury-excluded rows).
...
DECLARE
  ...
  v_label text;
BEGIN
  -- Resolve target round: explicit, else latest round with accuracy rows
  -- for the season where an actual score has been recorded.
  ...
  -- No accuracy data for the requested season/round: return empty set.
