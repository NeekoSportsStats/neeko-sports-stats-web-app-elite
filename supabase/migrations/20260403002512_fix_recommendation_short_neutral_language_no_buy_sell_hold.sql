...
/*
  # Fix recommendation_short — remove BUY/SELL/HOLD language

  ## Problem
  The `populate_rankings_cache_from_source()` function writes hardcoded
  `recommendation_short` values that contain "selling", "buy", "sell" etc.
  This causes 48 records to fail `ai_validation_passed` checks.

  ## Fix
  Replace the hardcoded recommendation_short strings in the cache populate
  function with neutral action-oriented language that avoids BUY/SELL/HOLD words.

  Old → New:
  - "Overpriced — consider selling"       → "Overpriced — price likely to drop"
  - "Below value — trade target"          → "Below value — consider trading"
  - "Not earning price — sell risk"       → "Not scoring to price — risk elevated"
  - "Strong buy signal"                   → "Strong value signal — price rising"
  - "Hold and monitor"                    → "Performing to price — monitor"

  Also wires ai_validation_passed into the ON CONFLICT DO UPDATE block
  so future cache refreshes maintain the field correctly.
*/
...
-- Neutral language — no BUY/SELL/HOLD words in human-facing text
CASE
...
-- ai_validation_passed: true when summary exists, has timestamp, no trade words
(
  ai_ana.analysis IS NOT NULL
  AND ai_ana.generated_at IS NOT NULL
  AND ai_ana.analysis NOT ILIKE '%buy%'
  AND ai_ana.analysis NOT ILIKE '%sell%'
  AND ai_ana.analysis NOT ILIKE '%hold%'
