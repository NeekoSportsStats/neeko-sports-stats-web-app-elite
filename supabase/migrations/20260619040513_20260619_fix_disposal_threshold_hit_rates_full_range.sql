/*
  # Fix disposal threshold hit rates: expand from 4 hardcoded values to full 10–40 range

  ## Problem
  get_stat_board_players returned only keys '15','20','25','30' in both
  all_threshold_hit_rates and season_threshold_hit_rates for the 'disposals' lens.
  The expanded player panel iterates publicExpandedPlayer = range(10, 40) (31 integers),
  so 27 of 31 key lookups returned undefined and displayed as "—".

  ## Fix
  Replace the 4 hardcoded COUNT FILTER columns in the agg CTE with a lateral
  generate_series approach. A new disp_hits CTE computes per-player hit counts
  for every integer threshold 10–40 inclusive, then disp_jsonb assembles the
  full JSONB maps via jsonb_object_agg. The canonical valid-game definition is
  unchanged — all denominators remain ag.cnt10 (last-10) and ag.total_g (season).
*/
...
/*
  disp_hits — disposal lens only (WHERE clause ensures zero rows otherwise).
  Computes hit counts for every integer threshold 10–40 inclusive via
  generate_series. Avoids 31 separate hardcoded COUNT FILTER expressions.
*/
disp_hits AS (
SELECT
  rg.pid                                                                  AS player_pid,
  t.threshold,
  COUNT(*) FILTER (WHERE rg.rn <= 10 AND rg.sv >= t.threshold)           AS last_10_hits,
  COUNT(*) FILTER (WHERE rg.sv >= t.threshold)                            AS season_hits
FROM ranked_games rg
CROSS JOIN generate_series(10, 40) AS t(threshold)
WHERE (SELECT lens FROM params) = 'disposals'
GROUP BY rg.pid, t.threshold
),
...
SELECT
  ...
  jsonb_object_agg(
    dh.threshold::text,
    jsonb_build_object(
      'hits',  dh.last_10_hits,
      'games', ag.cnt10,
      'rate',  ROUND(dh.last_10_hits::numeric / NULLIF(ag.cnt10, 0) * 100, 0)
    )
  ) AS last_10_map,
  jsonb_object_agg(
    dh.threshold::text,
    jsonb_build_object(
      'hits',  dh.season_hits,
      'games', ag.total_g,
      'rate',  ROUND(dh.season_hits::numeric / NULLIF(ag.total_g, 0) * 100, 0)
    )
  ) AS season_map
FROM disp_hits dh
JOIN agg ag ON ag.pid = dh.player_pid
GROUP BY dh.player_pid
),
...
CASE (SELECT lens FROM params)
WHEN 'disposals' THEN COALESCE(dj.last_10_map, jsonb_build_object())
WHEN 'goals' THEN
jsonb_build_object(
...
CASE (SELECT lens FROM params)
WHEN 'disposals' THEN COALESCE(dj.season_map, jsonb_build_object())
WHEN 'goals' THEN
jsonb_build_object(
...
LEFT JOIN disp_jsonb dj ON dj.player_pid = fp.player_id
