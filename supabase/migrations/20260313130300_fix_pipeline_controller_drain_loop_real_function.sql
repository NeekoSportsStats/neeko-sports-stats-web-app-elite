/*
  # Fix Pipeline Controller Drain Loop (Real No-Arg Function)

  ## Summary
  Replaces the drain_reco_queue step in the real run_afl_pipeline_controller() (no-arg version,
  called by cron) with a fixed loop that:
  1. Resets stuck processing jobs before starting
  2. Polls both pending + processing (active_now) so it waits for in-flight workers
  3. Only fires a new worker invocation when there are actually pending (unclaimed) jobs
  4. Sleeps 3s between iterations (was 1s) to give async workers time to claim jobs
  5. Hard cap raised to 50 iterations (was 30)

  ## Root Cause
  net.http_post is fire-and-forget. With 1s sleep the loop re-checked queue before any worker
  claimed jobs, saw pending > 0, fired another worker, and exited after 30 no-op iterations
  while 640 jobs remained untouched.
*/
...
DECLARE
  ...
  -- Fixed drain constants
  max_reco_iterations CONSTANT int     := 50;
...
--------------------------------------------------------------------------
-- STEP 7c: DRAIN RECO QUEUE (FIXED)
-- Changes from old version:
--   - Reset stuck processing jobs first
--   - Poll pending + processing (active_now) to detect in-flight workers
--   - Only fire new worker when pending > 0 (not just active > 0)
--   - Sleep 3s instead of 1s
--   - Hard cap 50 iterations instead of 30
--------------------------------------------------------------------------
v_step_started := now();
INSERT INTO public.pipeline_steps (run_id, step_name, step_label, status, started_at)
...
BEGIN
  ...
  IF pending_before > 0 THEN
    ...
    LOOP
      ...
      FROM public.ai_generation_queue
      WHERE job_type = 'ranking_recommendation' AND status = 'pending';

      -- Check processing jobs (in-flight with a worker)
      SELECT COUNT(*) INTO processing_now
      FROM public.ai_generation_queue
      WHERE job_type = 'ranking_recommendation' AND status = 'processing';
      ...
      -- Fire a worker only if there are unclaimed pending jobs
      ...
      -- Wait longer to give async worker time to claim and process jobs
      PERFORM pg_sleep(drain_sleep_secs);

      reco_loop_i := reco_loop_i + 1;

      -- Reset newly stuck jobs each iteration
      stuck_reset_count := stuck_reset_count + public.fn_reset_stuck_ai_queue_jobs();

      -- Hard safety cap
      EXIT WHEN reco_loop_i >= max_reco_iterations;
    END LOOP;
