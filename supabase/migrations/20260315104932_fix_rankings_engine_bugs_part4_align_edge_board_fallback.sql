/*
  # Fix Rankings Engine Bugs — Part 4: Align get_edge_board_data fallback logic

  ## Change
  The fallback path in get_edge_board_data() used:
    - breakout: upside_percentile >= 0.65 AND value_score > 100
    - trap: value_score < 95 (this was already correct in the fallback)

  Aligned to match the primary (mv_edge_board) path:
    - breakout: ceiling_gap >= 50 AND projection >= 50 AND floor >= 25
                AND confidence >= 40 AND risk <= 75 AND captain_rank > 5
    - breakout sort: upside_rating DESC, ceiling_gap DESC
    - trap: unchanged (was already correct at < 95)
*/
...
ELSE
  ...
  -- ALIGNED: now matches mv_edge_board ceiling_gap logic
  ...
  -- trap threshold was already correct at < 95 in the fallback
  trap_strict AS (
    SELECT * FROM ranked
  ...
  sectioned AS (
    ...
    SELECT
      ...
      -- ALIGNED: upside_rating DESC, ceiling_gap DESC (matches primary path)
