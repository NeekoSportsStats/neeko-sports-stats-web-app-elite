/*
  # Rebuild get_edge_board_data RPC — v2 (reads from mv_edge_board)

  ## Summary
  Drops and recreates get_edge_board_data() to:
  1. Read from mv_edge_board materialized view for fast responses (<5ms)
  2. Fall back to live view chain if mv is empty (safety net on first deploy)
  3. Add refreshed_at column so the frontend can display "last updated" time

  Must DROP first because the return type is changing (adding refreshed_at).
*/
