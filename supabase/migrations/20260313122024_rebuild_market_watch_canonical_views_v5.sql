/*
  # Market Watch Canonical Views Rebuild v5

  Complete rebuild with 4-category model (buy/sell/cash_cow/trap),
  normalized trade scores 0-100, trade pair engine, status/summary views.
  Fixed: all COALESCE type mismatches, explicit casts throughout.
*/
...
WITH base AS (
  SELECT
    ...
    ROUND((rc.price::numeric / 47.7), 1)                     AS breakeven,
    ROUND(
      GREATEST(rc.projection_final::numeric, 0::numeric) - (rc.price::numeric / 47.7), 1
    )                                                         AS price_edge_pts,
    ROUND(
      LEAST(
        GREATEST(
          (GREATEST(rc.projection_final::numeric, 0::numeric) - (rc.price::numeric / 47.7)) * 47.7,
  ...
  SELECT
    ...
    CASE
      WHEN p.price > 700000
        ...
        AND p.projection < (p.price::numeric / 47.7) * 1.1
        THEN 'trap'
      WHEN p.price > 450000
        ...
        THEN 'sell'
      WHEN p.price <= 350000
        AND p.projection >= p.breakeven
        AND p.form_score > p.breakeven
        THEN 'cash_cow'
      WHEN p.price_edge_pts >= 10
        AND p.value_tier IN ('STRONG VALUE','GOOD VALUE','FAIR VALUE','UNDERPRICED')
        AND p.projection_confidence >= 60
        THEN 'buy'
      WHEN p.value_score >= 9.5 AND p.projection_confidence >= 70 AND p.price <= 900000
        THEN 'buy'
      WHEN p.price <= 300000 AND p.price_edge_pts > 0
        THEN 'cash_cow'
      WHEN p.price > 350000 AND p.price_edge_pts < -10 AND p.risk_pct > 50
        THEN 'sell'
      WHEN p.price > 900000 AND p.projection < p.price::numeric / 60.0
        THEN 'trap'
      WHEN p.price_edge_pts > 0 THEN 'buy'
      ELSE 'sell'
    ...
    CASE
      ...
      WHEN p.price > 450000 AND p.value_tier = 'OVERPRICED' AND p.price_edge_pts < 5
        THEN 'Projection doesn''t justify price tier'
      WHEN p.price <= 350000 AND p.projection >= p.breakeven AND p.form_score > p.breakeven
        THEN 'Low-cost player tracking above breakeven'
      WHEN p.price_edge_pts >= 10
        AND p.value_tier IN ('STRONG VALUE','GOOD VALUE','FAIR VALUE','UNDERPRICED')
        THEN 'Projects above cost — strong value signal'
      WHEN p.value_score >= 9.5
        THEN 'High value score relative to market price'
      WHEN p.price <= 300000 AND p.price_edge_pts > 0
        THEN 'Budget player generating positive price growth'
      WHEN p.price_edge_pts < -10 AND p.risk_pct > 50
        THEN 'High risk and projection below breakeven'
      WHEN p.price > 900000 AND p.projection < p.price::numeric / 60.0
        THEN 'Premium price but projection disappoints'
      WHEN p.price_edge_pts > 0 THEN 'Projecting above breakeven'
...
WITH best_trade AS (
  ...
  ORDER BY projected_points_gain DESC LIMIT 1
),
best_cow AS (
  SELECT
    ...
    price                 AS out_price,
    NULL::int             AS in_price,
    season, round_number, snapshot_updated_at
  FROM public.v_mw_premium
  WHERE category = 'cash_cow'
  ORDER BY trade_score DESC LIMIT 1
),
biggest_trap AS (
  SELECT
  ...
  FROM public.v_mw_premium
  WHERE category = 'trap'
  ORDER BY trade_score ASC LIMIT 1
)
