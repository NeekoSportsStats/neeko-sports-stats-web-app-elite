...
export default function NBACompleteAIAnalysis() {
  ...
  const fetchAIAnalysis = async () => {
    try {
      ...
      if (!data || data.length === 0) {
        setError("No NBA AI insights available yet. Admin can resync data to generate insights.");
        setBlocks([]);
        return;
      }
      ...
      data.forEach((item) => {
        ...
        const statLabel = item.stat_label || "Points";
      ...
      console.log("Loaded", blocksArray.length, "NBA AI analysis blocks with", data.length, "total players");
    } catch (err) {
      console.error("Error in fetchAIAnalysis:", err);
      setError(err instanceof Error ? err.message : "Failed to load NBA AI analysis");
      setBlocks([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Generating NBA AI insights...",
      });
      ...
      toast({
        title: "Success",
        description: data.message || "NBA AI analysis refreshed successfully",
      });

      // Refresh the data
      await fetchAIAnalysis();
    } catch (err) {
      console.error("Error refreshing NBA AI analysis:", err);
      toast({
        title: "Refresh Failed",
        description: err instanceof Error ? err.message : "Failed to refresh NBA AI analysis",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold mb-2">NBA AI Analysis</h1>
