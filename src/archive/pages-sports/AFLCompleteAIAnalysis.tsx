...
export default function AFLCompleteAIAnalysis() {
  ...
  const fetchAIInsights = async () => {
    try {
      ...
      if (!data || data.length === 0) {
        setError("No AI insights available yet. Admin can resync data to generate insights.");
        setBlocks([]);
        return;
      }
      ...
      data.forEach((item) => {
        ...
        const statLabel = item.stat_label || "Fantasy Score";
      ...
      console.log("Loaded", blocksArray.length, "AI analysis blocks with", data.length, "total players");
    } catch (err) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      toast({
        title: "Syncing Data",
        description: "Syncing AFL data from Google Sheet...",
      });
      
      const syncResponse = await supabase.functions.invoke("sync-googlesheet", {
        headers: {
      ...
      toast({
        ...
        description: "Generating AFL AI insights...",
      });
      
      const aiResponse = await supabase.functions.invoke("generate-ai-insights", {
        headers: {
          Authorization: `Bearer ${session.access_token}`,
        },
      });
      ...
      toast({
        title: "Success",
        description: "AFL AI insights refreshed successfully",
      });

      // Refresh the data
      await fetchAIInsights();
    } catch (err) {
      console.error("Error refreshing AFL AI insights:", err);
      toast({
        title: "Refresh Failed",
        description: err instanceof Error ? err.message : "Failed to refresh AFL AI insights",
        variant: "destructive",
      });
    } finally {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold mb-2">AFL AI Analysis</h1>
