...
// The AI text comes from afl.player_rankings_cache which is generated by
// a fantasy-framing prompt. Any text that contains fantasy-specific language
// is suppressed here — the stat board shows a neutral placeholder instead.
// recommendation_short and recommendation_color are never used on stat board.
...
function AiInsightBlock({
  ...
  // Use summary_long first, fall back to summary_short.
  // If the text contains any fantasy decision/pricing language it is treated
  // as unavailable — it was generated for the rankings page, not stat board.
...
// Tooltip data uses viewport (screen-space) coordinates so the portal can
// place it with position:fixed — fully outside any overflow/clip context.
  ...
  return (
    <div className="w-full relative" onMouseLeave={() => setHovered(null)}>
      ...
      >
        ...
        {/* Invisible hit-zone rects — record screen-space coords on enter */}
        {slots.map((slot, i) => {
          const cx = xOf(i);
      ...
      {/* DOM tooltip — rendered via portal so it escapes all overflow:hidden ancestors */}
...
function ChartTooltip({
  ...
  // Horizontal: prefer centred on cursor, clamp to viewport edges
  ...
  // Height: header (48) + divider (1) + rows (allThresholds.length × 22) + footer (22) + padding (12)
...
/** Format to one decimal place, "—" for missing */
